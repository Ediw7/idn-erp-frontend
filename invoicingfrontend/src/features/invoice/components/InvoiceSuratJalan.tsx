...
export const InvoiceSuratJalan: React.FC<InvoiceSuratJalanProps> = ({
  form,
  setForm,
  suratJalans,
  salesOrders,
  ...
  const availableSJs = suratJalans.filter((sj) => {
    ...
    const alreadyInForm = (form.surat_jalans || []).find(
      (s: any) => s.no_sj === sj.no_sj,
    );
  ...
  const handleAddSj = () => {
    ...
    targetSJ.lines.forEach((sjLine: any) => {
      ...
      const existingLineIndex = newLines.findIndex(
        (l: any) => l.item_id === sjLine.item_id,
      );
    ...
    setForm((prev: any) => ({
      ...
      surat_jalans: [
        ...(prev.surat_jalans || []),
        {
          no_sj: targetSJ.no_sj,
          tanggal: targetSJ.tanggal,
          keterangan: targetSJ.keterangan || "",
        },
      ],
    }));
  ...
  const handleRemoveSj = (index: number) => {
    ...
    toast.error(
      "Menghapus SJ dari daftar tidak otomatis menghapus/mengurangi barang dari tab Detail. Harap sesuaikan kuantum barang secara manual di tab Detail jika diperlukan.",
    );

    setForm((prev: any) => ({
      ...prev,
      surat_jalans: prev.surat_jalans.filter(
        ...
        <div className="flex flex-col flex-1 max-w-sm gap-1">
          ...
          >
            <option value="">
              {form.no_so
                ? "-- Pilih Surat Jalan --"
                : "-- Pilih SO di Umum dulu --"}
            </option>
            {availableSJs.map((sj) => (
              <option key={sj.id} value={sj.no_sj}>
                {sj.no_sj} ({sj.tanggal})
              </option>
            ))}
          </select>
      ...
      <table className="w-full text-sm text-left whitespace-nowrap mt-2 border border-slate-200">
        <thead className="bg-slate-100 border-b border-slate-300 text-slate-700">
          <tr>
            ...
            <th className="px-4 py-2 border-r border-slate-300 font-semibold w-48 text-center">
              No. SJ
            </th>
            <th className="px-4 py-2 border-r border-slate-300 font-semibold w-32 text-center">
              Tgl SJ
            </th>
        ...
        <tbody className="divide-y divide-slate-200 bg-white">
          {form.surat_jalans && form.surat_jalans.length > 0 ? (
            form.surat_jalans.map((sj: any, idx: number) => (
              <tr key={idx} className="hover:bg-blue-50 transition-colors">
                <td className="px-4 py-2 border-r border-slate-200 text-center">
                  {idx + 1}
                </td>
                <td className="px-4 py-2 border-r border-slate-200 font-mono text-center">
                  {sj.no_sj}
                </td>
                <td className="px-4 py-2 border-r border-slate-200 text-center">
                  {sj.tanggal}
            ...
            <tr>
              ...
              >
                Belum ada Surat Jalan terkait untuk Invoice ini.
              </td>
            </tr>
          )}
        </tbody>
