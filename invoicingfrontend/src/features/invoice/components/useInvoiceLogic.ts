...
export const useInvoiceLogic = (locationSearch: string) => {
    ...
    if (
      ...
      if (targetSJ) {
        ...
        setForm((prev: any) => ({
          ...
          surat_jalans: [
            {
              ...
              keterangan:
                targetSJ.keterangan || `Auto-generated from ${targetSJ.no_sj}`,
            },
          ],
          ...
          mata_uang: targetSO?.mata_uang_id
            ? String(targetSO.mata_uang_id)
            : prev.mata_uang,
          salesman_id: targetSO?.salesman_id
            ? String(targetSO.salesman_id)
            : prev.salesman_id,
          cara_pembayaran: targetSO?.pembayaran_id
            ? String(targetSO.pembayaran_id)
            : prev.cara_pembayaran,
      ...
      if (targetSO) {
        setForm((prev: any) => ({
          ...
          lines: targetSO.lines.map((l: any) => ({
            ...
            harga_jual:
              l.harga_jual ||
              l.kuantum * (l.harga_satuan || 0) - (l.disc_harga || 0),
          })),
    ...
    if (form.no_so && salesOrders.length > 0) {
      ...
      if (targetSO) {
        setForm((prev: any) => ({
          ...
          mata_uang:
            prev.mata_uang ||
            (targetSO.mata_uang_id ? String(targetSO.mata_uang_id) : "IDR"),
          salesman_id:
            prev.salesman_id ||
            (targetSO.salesman_id ? String(targetSO.salesman_id) : ""),
          cara_pembayaran:
            prev.cara_pembayaran ||
            (targetSO.pembayaran_id ? String(targetSO.pembayaran_id) : ""),
          proyek:
            prev.proyek || (targetSO.proyek ? String(targetSO.proyek) : ""),
  ...
  const handleCreateInvoiceHeader = () => {
    ...
    toast.success(
      "Header Invoice berhasil dibuat. Silakan lengkapi detail barang.",
    );
  };
  ...
  const handleSaveAll = async () => {
    if (form.is_lunas || form.is_void) {
      toast.error(
        "Invoice sudah Lunas atau Void sehingga tidak bisa disimpan / diubah!",
      );
      return;
    }
    if (!form.no_invoice) {
