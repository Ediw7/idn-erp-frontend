      ...
      <div className="flex flex-col lg:flex-row gap-5">
        ...
        <div className="flex-1 flex flex-col gap-3">
          <div className="flex items-start">
            ...
            >
              <option value={form.no_invoice}>
                {form.no_invoice || "-- Pilih --"}
              </option>
              {dataList
                .filter((x) => x.no_invoice !== form.no_invoice)
                .map((inv) => (
          ...
          <div className="flex items-start">
            ...
            <input
              ...
              value={form.tgl_invoice || ""}
              onChange={(e) =>
                setForm({ ...form, tgl_invoice: e.target.value })
              }
            />
          </div>
          <div className="flex items-start">
            ...
            <div className="flex gap-2 w-full">
              ...
              <button
                className="px-3 border border-slate-300 bg-slate-100 hover:bg-slate-200 rounded-sm font-bold text-slate-600 transition-colors disabled:opacity-50"
          ...
          <div className="flex items-start">
            ...
            >
              <option value="">
                {loadingData ? "Loading..." : "-- Pilih Proyek --"}
              </option>
              {proyeks.map((p) => (
          ...
          <div className="flex items-start">
            ...
            <input
              ...
              onChange={(e) => setForm({ ...form, kurs_jual: e.target.value })}
            />
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col gap-3">
          <div className="flex items-start">
            ...
            <div className="flex gap-1 flex-1">
              <select
                className={inputClass}
                value={form.no_so || ""}
                onChange={(e) => setForm({ ...form, no_so: e.target.value })}
              >
                <option value="">
                  {loadingData ? "Loading..." : "-- Pilih SO --"}
                </option>
                {salesOrders.map((so, idx) => (
                  <option key={idx} value={so.no_so}>
                    {so.no_so}
                  </option>
                ))}
              </select>
          ...
          <div className="flex items-start">
            ...
            <input
              ...
              className={`${inputClass} w-48`}
              value={form.tgl_po || ""}
              onChange={(e) => setForm({ ...form, tgl_po: e.target.value })}
            />
          </div>
          <div className="flex items-start">
            ...
            <input
              ...
              className={`${inputClass} w-48`}
              value={form.tgl_jt || ""}
              onChange={(e) => setForm({ ...form, tgl_jt: e.target.value })}
            />
          </div>
          <div className="flex items-start">
          ...
          <div className="flex items-start">
            ...
            <select
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  salesman_id: e.target.value ? Number(e.target.value) : "",
                })
              }
            >
              <option value="">
          ...
          <div className="flex items-start">
            ...
            <select
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  gudang_id: e.target.value ? Number(e.target.value) : "",
                })
              }
            >
              <option value="">
          ...
          <div className="flex items-start mt-2 border-t border-slate-200 pt-3">
            ...
            <div className="flex gap-1 flex-1">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, no_faktur_pajak: e.target.value })
                }
              />
              <button className="px-3 py-1.5 border border-slate-300 bg-slate-100 hover:bg-slate-200 rounded-sm">
                <RefreshCcw size={14} />
              </button>
            </div>
          </div>
        ...
        <div className="w-[250px] flex flex-col gap-4">
          <div className="flex gap-4 items-center bg-slate-50 p-3 border border-slate-200 rounded-sm">
            ...
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700 cursor-pointer">
              <input
                type="checkbox"
                checked={form.is_lunas || false}
                disabled
                className="w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 opacity-60 cursor-not-allowed"
              />{" "}
              Closed (Lunas) ?
            </label>
          </div>
          ...
          <div className="border border-slate-300 p-3 relative mt-4 rounded-sm bg-white shadow-sm">
            <span className="absolute -top-2 left-3 bg-white px-1 text-[10px] font-bold text-slate-500 uppercase tracking-wide">
              Record Created
            </span>
            <div className="flex gap-2 mt-2">
              <input
                type="text"
                className="w-full px-2 py-1.5 text-xs border border-slate-200 bg-slate-50 font-mono rounded-sm"
                readOnly
                value={
                  form.create_date
                    ? new Date(form.create_date).toLocaleString("id-ID")
                    : "-"
                }
              />
              <input
                type="text"
                className="w-20 px-2 py-1.5 text-xs border border-slate-200 bg-slate-50 text-center rounded-sm"
                readOnly
                value={form.create_uid_name || "Admin"}
              />
            </div>
          </div>
          <div className="border border-slate-300 p-3 relative mt-3 rounded-sm bg-white shadow-sm">
            ...
            <div className="flex gap-2 mt-2">
              <input
                ...
                value={
                  form.write_date
                    ? new Date(form.write_date).toLocaleString("id-ID")
                    : "-"
                }
              />
              <input
                ...
                className="w-20 px-2 py-1.5 text-xs border border-slate-200 bg-slate-50 text-center rounded-sm"
                readOnly
                value={form.write_uid_name || "Admin"}
              />
            </div>
          </div>
        </div>
