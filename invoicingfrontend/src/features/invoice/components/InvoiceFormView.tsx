  ...
  return (
    <div className="bg-slate-50 shadow-sm border border-slate-300 flex flex-col h-[calc(100vh-8rem)]">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={async () => {
              ...
              setModalForm({ ...emptyForm, no_invoice: autoNo });
              setShowNewInvoiceModal(true);
            }}
            className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-slate-800 bg-white border border-transparent hover:bg-slate-100 transition-colors rounded-sm shadow-sm whitespace-nowrap"
          >
          ...
          <button
            onClick={() =>
              navigate(
                form.no_invoice
                  ? `/laporan?invoice_number=${encodeURIComponent(form.no_invoice)}&reportName=${encodeURIComponent("Invoice (A4 / Kuarto)")}`
                  : "/laporan?reportName=Invoice (A4 / Kuarto)",
              )
            }
            className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-bold text-slate-800 bg-white border border-transparent hover:bg-slate-100 transition-colors rounded-sm shadow-sm whitespace-nowrap"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white bg-green-600 border border-transparent hover:bg-green-700 transition-colors rounded-sm shadow-sm whitespace-nowrap"
          >
            PERBARUI FP
          </button>
          <button
      ...
      <div className="bg-white border-t border-slate-300 p-4 shrink-0 flex justify-end gap-3 z-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <button
          className={`${btnClass} bg-white text-slate-700 border border-slate-300 hover:bg-slate-50 px-6 py-2 text-sm`}
        >
          BATAL
        </button>
        {!isLocked && (
          <button
            ...
            className={`${btnClass} bg-blue-600 text-white hover:bg-blue-700 px-8 py-2 text-sm shadow-sm`}
          >
