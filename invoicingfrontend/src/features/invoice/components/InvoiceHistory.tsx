    ...
    <div className="flex gap-6 min-h-[300px]">
      <div className="flex-1 bg-white border border-slate-200 shadow-sm flex flex-col rounded-sm">
        <table className="w-full text-sm text-left whitespace-nowrap">
          ...
          <tbody className="divide-y divide-slate-200 bg-white">
            {form.histories && form.histories.length > 0 ? (
              form.histories.map((h: any, idx: number) => (
                <tr key={idx} className="hover:bg-blue-50 transition-colors">
                  ...
                  <td className="px-4 py-2 text-right font-mono text-blue-800 font-semibold">
                    {(h.pembayaran || 0).toLocaleString("en-US", {
      ...
      <div className="w-64 shrink-0 flex flex-col gap-2">
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm p-4">
          <label className="block text-sm font-semibold text-slate-700 mb-2 border-b border-slate-200 pb-2">
            Sisa Piutang
          </label>
