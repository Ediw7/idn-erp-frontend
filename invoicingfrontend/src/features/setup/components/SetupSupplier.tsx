...
const SetupSupplier: React.FC = () => {
  ...
  const {
    ...
    validate: (form) =>
      !form.kode || !form.nama ? "Kode dan Nama Supplier harus diisi!" : null,
  });

  const inputClass =
    "w-full px-2 py-1 bg-white border border-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-500/20 focus:border-slate-500 text-xs transition-colors rounded-sm disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed";
  ...
  const handleExportExcel = () => {
    const exportData = list.map((item) => ({
      ...
      "Jenis Transaksi":
        JENIS_TRANSAKSI.find((j) => j.value === item.jenis_transaksi)?.label ||
        item.jenis_transaksi ||
        "",
      "Cara Pembayaran":
        pembayarans.find((p) => p.id === item.pembayaran_id)?.nama || "",
      ...
      try {
        ...
        const itemsToImport = data
          .map((row) => {
            ...
            const matchedJenis = JENIS_TRANSAKSI.find(
              (j) =>
                j.label === jenisTransaksiLabel ||
                j.value === jenisTransaksiLabel,
            );

            const caraPembayaranNama = row["Cara Pembayaran"] || "";
            const matchedPembayaran = pembayarans.find(
              (p) => p.nama === caraPembayaranNama,
            );
            ...
            return {
              ...
              telepon:
                row["Nomor Telepon"] || row["Telepon"] || row["TELEPON"] || "",
              ...
              is_pkp:
                (row["Apakah PKP ?"] || "").toString().toLowerCase() === "ya",
        ...
        const importPromise = Promise.all(
          itemsToImport.map((item) =>
            setupApi.saveSupplier(item as SupplierData),
          ),
        );
      ...
      <div className="bg-slate-800 px-6 py-4 flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-lg font-semibold text-white">
            Setup Supplier (Vendor)
          </h2>
          <p className="text-xs text-slate-300 mt-1">
            Daftar pemasok barang/jasa untuk keperluan pembelian.
          </p>
        </div>
              ...
              <div className="flex gap-8">
                ...
                <div className="flex-1 flex flex-col gap-3">
                  <div className="flex items-center">
                    <label className="w-36 text-xs font-semibold text-slate-700 shrink-0">
                      Kode Supplier
                    </label>
                    <input
                      type="text"
                  ...
                  <div className="flex items-start">
                    <label className="w-36 text-xs font-semibold text-slate-700 shrink-0 pt-1.5">
                      Alamat Supplier
                    </label>
                    <textarea
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 flex items-center">
                      ...
                      <input
                        ...
                        value={editForm.fax || ""}
                        onChange={(e) =>
                          setEditForm({ ...editForm, fax: e.target.value })
                        }
                        className={inputClass}
                      />
                    </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="w-32 flex items-center">
                      <label className="w-16 text-xs font-semibold text-slate-700 shrink-0 pr-3">
                        No. HP
                      </label>
                      <input
                        type="text"
                        ...
                        onChange={(e) =>
                          setEditForm({ ...editForm, no_hp: e.target.value })
                        }
                        className={inputClass}
                      />
                    </div>
                ...
                <div className="flex-1 flex flex-col gap-3">
                  <div className="flex items-center">
                    <label className="w-40 text-xs font-semibold text-slate-700 shrink-0">
                      Nama Wajib Pajak
                    </label>
                    <input
                      type="text"
                      value={editForm.nama_wp || ""}
                      onChange={(e) =>
                        setEditForm({ ...editForm, nama_wp: e.target.value })
                      }
                      className={inputClass}
                    />
                  </div>
                  <div className="flex items-start">
                    <label className="w-40 text-xs font-semibold text-slate-700 shrink-0 pt-1.5">
                      Alamat Wajib Pajak
                    </label>
                    <textarea
                      value={editForm.alamat_wp || ""}
                      onChange={(e) =>
                        setEditForm({ ...editForm, alamat_wp: e.target.value })
                      }
                      className={`${inputClass} h-16 resize-none`}
                    />
                  </div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setEditForm({
                            ...editForm,
                            is_pkp: checked,
                            tgl_pengukuhan: checked
                              ? editForm.tgl_pengukuhan
                              : "",
                            no_seri_fp_masukan: checked
                              ? editForm.no_seri_fp_masukan
                              : "",
                          });
                        }}
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 flex items-center">
                      ...
                      <input
                        disabled={!editForm.is_pkp}
                        type="date"
                        value={editForm.tgl_pengukuhan || ""}
                        onChange={(e) =>
                          setEditForm({
                            ...editForm,
                            tgl_pengukuhan: e.target.value,
                          })
                        }
                        className={inputClass}
                      />
                  ...
                  <div className="flex items-center">
                    ...
                    <input
                      disabled={!editForm.is_pkp}
                      type="text"
                      value={editForm.no_seri_fp_masukan || ""}
                      onChange={(e) =>
                        setEditForm({
                          ...editForm,
                          no_seri_fp_masukan: e.target.value,
                        })
                      }
                      className={inputClass}
                      placeholder={
                        editForm.is_pkp ? "" : "Centang PKP terlebih dahulu"
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center mt-2">
                    ...
                    <select
                      value={editForm.jenis_transaksi || "01"}
                      onChange={(e) =>
                        setEditForm({
                          ...editForm,
                  ...
                  <div className="flex items-center">
                    ...
                    <select
                      value={editForm.pembayaran_id || ""}
                      onChange={(e) =>
                        setEditForm({
                          ...editForm,
                          pembayaran_id: e.target.value
