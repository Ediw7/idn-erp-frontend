...
const SetupMataUang: React.FC = () => {
  const {
    ...
    handleSave,
    handleDelete,
  } = useMasterDataCRUD<MataUangData>({
    fetchApi: setupApi.getMataUang,
    saveApi: setupApi.saveMataUang,
    deleteApi: setupApi.deleteMataUang,
    initialForm: { kode: "", nama: "", per: "" },
    validate: (form) =>
      !form.kode || !form.nama ? "Kode dan Mata Uang harus diisi!" : null,
  });
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">
            Konfigurasi jenis mata uang yang digunakan dalam sistem.
          </p>
        </div>
        <button
              ...
              <tbody className="text-sm text-slate-700 divide-y divide-slate-100">
                ...
                {mataUangList.length === 0 && (
                  <tr>
                    ...
                    >
                      Belum ada data mata uang. Klik "Tambah Baru" untuk
                      memulai.
                    </td>
                  </tr>
                )}
          ...
          <div className="bg-white rounded shadow-xl max-w-md w-full flex flex-col">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800">
                {editForm.id ? "UBAH Mata Uang" : "Tambah Mata Uang"}
              </h3>
              <button
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
                        per: e.target.value.toUpperCase(),
                      })
                    }
                    className={inputClass}
