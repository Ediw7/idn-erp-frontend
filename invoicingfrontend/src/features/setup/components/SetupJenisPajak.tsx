...
const SetupJenisPajak: React.FC = () => {
  const {
    ...
    handleSave,
    handleDelete,
  } = useMasterDataCRUD<JenisPajakData>({
    fetchApi: setupApi.getJenisPajak,
    saveApi: setupApi.saveJenisPajak,
    deleteApi: setupApi.deleteJenisPajak,
    initialForm: { kode: "", nama: "" },
    validate: (form) =>
      !form.kode || !form.nama ? "Kode dan Jenis Pajak harus diisi!" : null,
  });
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-lg font-semibold text-white">
            Setup Kode Jenis Pajak
          </h2>
          <p className="text-xs text-slate-300 mt-1">
            Daftar kode jenis pajak seperti PPh, PPN, dll.
          </p>
        </div>
        <button
          ...
          <div className="bg-white rounded shadow-xl max-w-md w-full flex flex-col">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
              <h3 className="font-bold text-slate-800">
                {editForm.id ? "UBAH Jenis Pajak" : "Tambah Jenis Pajak"}
              </h3>
              <button
