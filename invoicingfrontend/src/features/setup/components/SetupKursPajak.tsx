...
const SetupKursPajak: React.FC = () => {
  ...
  const fetchKursPajak = () => {
    if (selectedMataUangId)
      return setupApi.getKursPajak(Number(selectedMataUangId));
  ...
  const {
    ...
    validate: (form) =>
      !form.tgl_dari || !form.tgl_sd || !form.kurs
        ? "Tanggal Dari, Tanggal s/d, dan Kurs harus diisi!"
        : null,
  });
  ...
  const selectedCurrencyInfo = mataUangList.find(
    (m) => m.id === Number(selectedMataUangId),
  );
  const inputClass =
    ...
    <div className="bg-white shadow-sm border border-slate-300 max-w-5xl mx-auto mt-8">
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">
            Kelola data kurs pajak per mata uang dan rentang waktu.
          </p>
        </div>
        <button
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-6 py-4 flex items-center gap-4">
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedMataUangId(e.target.value ? Number(e.target.value) : "")
          }
        >
              ...
              <tbody className="text-sm text-slate-700 divide-y divide-slate-100">
                {kursPajakList.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right font-medium">
                      {Number(item.kurs).toLocaleString("id-ID", {
                ...
                {kursPajakList.length === 0 && (
                  <tr>
                    ...
                    >
                      Belum ada data kurs pajak. Klik "Tambah Baru" untuk
                      memulai.
                    </td>
                  </tr>
                )}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="date"
                      value={editForm.tgl_dari}
                      onChange={(e) =>
                        setEditForm({ ...editForm, tgl_dari: e.target.value })
                      }
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      value={editForm.tgl_sd}
                      onChange={(e) =>
                        setEditForm({ ...editForm, tgl_sd: e.target.value })
                      }
                      className={inputClass}
                    />
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    value={editForm.kurs}
                    onChange={(e) =>
                      setEditForm({ ...editForm, kurs: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={editForm.no_kmk}
                    onChange={(e) =>
                      setEditForm({ ...editForm, no_kmk: e.target.value })
                    }
                    className={inputClass}
                    placeholder="Nomor KMK"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                  ...
                  <input
                    type="date"
                    value={editForm.tgl_kmk}
                    onChange={(e) =>
                      setEditForm({ ...editForm, tgl_kmk: e.target.value })
                    }
                    className={inputClass}
                  />
                </div>
