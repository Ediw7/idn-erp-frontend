...
const SetupJenisSetoran: React.FC = () => {
  ...
  const {
    ...
    validate: (form) =>
      !form.jenis_pajak_id || !form.kode || !form.nama
        ? "MAP, Kode, dan Jenis Setoran harus diisi!"
        : null,
  });
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-lg font-semibold text-white">
            Setup Kode Jenis Setoran
          </h2>
          <p className="text-xs text-slate-300 mt-1">
            Daftar kode jenis setoran (KJS) yang terhubung dengan MAP (Kode
            Jenis Pajak).
          </p>
        </div>
        <button
                ...
                {list.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-semibold">
                      {item.jenis_pajak_kode}
                    </td>
                    <td className="px-4 py-3 text-slate-600">
                      {item.jenis_pajak_nama}
                    </td>
