...
const SetupBahasa: React.FC = () => {
  const {
    ...
    validate: (form) =>
      !form.jenis_objek || !form.nama_objek || !form.default_sistem
        ? "Jenis, Nama Objek, dan Default Sistem harus diisi!"
        : null,
  });
  ...
  const filteredList = useMemo(() => {
    return list.filter((item) => {
      const matchJenis = filterJenis
        ? item.jenis_objek.toLowerCase().includes(filterJenis.toLowerCase())
        : true;
      const matchNama = filterNama
        ? item.nama_objek.toLowerCase().includes(filterNama.toLowerCase())
        : true;
  ...
  const uniqueNama = Array.from(
    new Set(
      list
        .filter((item) => !filterJenis || item.jenis_objek === filterJenis)
        .map((item) => item.nama_objek),
    ),
  );
    ...
    <div className="bg-white shadow-sm border border-slate-300 w-full mx-auto mt-4 flex flex-col h-full">
      ...
      <div className="bg-slate-800 px-6 py-4 flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">
            Konfigurasi penerjemahan/perubahan judul label standar sistem.
          </p>
        </div>
      ...
      <div className="bg-blue-50/50 p-4 border-b border-slate-200 shrink-0 flex items-center gap-6">
        <div className="flex items-center gap-3">
          <label className="text-sm font-semibold text-blue-900 w-32">
            Cari Jenis Objek
          </label>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, jenis_objek: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, nama_objek: e.target.value })
                    }
                    className={inputClass}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setEditForm({ ...editForm, judul_kustom: e.target.value })
                    }
                    className={inputClass}
