...
const SetupItem: React.FC = () => {
  ...
  const {
    ...
    validate: (form) =>
      !form.kode || !form.nama ? "Kode dan Nama Item harus diisi!" : null,
  });
  ...
  const filteredList = list.filter((item) => {
    const matchKode = item.kode
      ?.toLowerCase()
      .includes(filterKode.toLowerCase());
    const matchNama = item.nama
      ?.toLowerCase()
      .includes(filterNama.toLowerCase());
    const matchGroup = filterGroup
      ? item.group_barang_id === Number(filterGroup)
      : true;
    return matchKode && matchNama && matchGroup;
  });

  const ic =
    "w-full px-2 py-1.5 bg-white border border-slate-300 focus:outline-none focus:ring-1 focus:ring-blue-500 text-xs transition-colors rounded-sm";
  const fmtCur = (val: number | undefined | null) =>
    val ? val.toLocaleString("id-ID", { minimumFractionDigits: 2 }) : "";
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-lg font-semibold text-white">
            Setup Item (Master Produk)
          </h2>
          <p className="text-xs text-slate-300 mt-1">
            Kelola data barang/jasa yang dijual atau dibeli perusahaan.
          </p>
        </div>
        <button
          onClick={handleAddNew}
          className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-800 bg-white hover:bg-slate-100 transition-colors"
        >
          <Plus size={14} />
      ...
      <div className="bg-slate-50 border-b border-slate-200 px-6 py-3 flex items-center gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-4 py-1.5 bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 text-xs font-semibold transition-colors rounded-sm shadow-sm"
          >
            TAMPILKAN SEMUA
          </button>
        </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-200 border-b border-slate-300 text-xs font-bold text-slate-800 uppercase tracking-wide">
                    <th className="px-2 py-2 border-r border-slate-300 w-24">
                      Kode
                    </th>
                    <th className="px-2 py-2 border-r border-slate-300 w-48">
                      Nama Item
                    </th>
                    <th className="px-2 py-2 border-r border-slate-300 w-28">
                      Group Item
                    </th>
                    <th className="px-2 py-2 border-r border-slate-300 w-16 text-center">
                      Satuan
                    </th>
                    <th className="px-2 py-2 border-r border-slate-300 w-28 text-right">
                      Harga Jual 1
                    </th>
                    <th className="px-2 py-2 border-r border-slate-300 w-28 text-right">
                      Harga Jual 2
                    ...
                    <th className="px-2 py-2 border-r border-slate-300 w-32">
                      Supplier Utama
                    </th>
                    <th className="px-2 py-2 border-r border-slate-300 w-24">
                      Perk Penjualan
                ...
                <tbody className="text-sm text-slate-800 divide-y divide-slate-200">
                  {filteredList
                    ...
                    .map((item) => (
                      ...
                      >
                        <td className="px-2 py-1.5 border-r border-slate-200 font-mono">
                          {item.kode}
                        </td>
                        <td className="px-2 py-1.5 border-r border-slate-200 font-medium">
                          {item.nama}
                        </td>
                        ...
                        <td className="px-2 py-1.5 border-r border-slate-200 text-right">
                          {fmtCur(item.harga_jual_1)}
                        </td>
                        <td className="px-2 py-1.5 border-r border-slate-200 text-right">
                          {fmtCur(item.harga_jual_2)}
                        </td>
                        <td className="px-2 py-1.5 border-r border-slate-200 text-right">
                          {fmtCur(item.harga_jual_3)}
                        </td>
                        <td
                          className="px-2 py-1.5 border-r border-slate-200 truncate max-w-[120px]"
                          title={item.supplier_utama}
                        >
                          {item.supplier_utama || "-"}
                        </td>
                        <td
                          className="px-2 py-1.5 border-r border-slate-200 text-xs truncate max-w-[100px]"
                          title={item.perk_penjualan_nama}
                        >
                          {item.perk_penjualan_nama || "-"}
                        </td>
                        <td
                          className="px-2 py-1.5 border-r border-slate-200 text-xs truncate max-w-[100px]"
                          title={item.perk_hpp_nama}
                        >
                          {item.perk_hpp_nama || "-"}
                        </td>
                        <td className="px-2 py-1.5 border-r border-slate-200 text-center">
                          <input
                            ...
                            readOnly
                            className="w-3.5 h-3.5 opacity-60 cursor-not-allowed"
                          />
                        </td>
          ...
          <div className="bg-white shadow-xl max-w-4xl w-full flex flex-col my-auto rounded-sm">
            <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-800">
              <h3 className="font-bold text-white">
                {editForm.id ? "UBAH Item" : "Tambah Item Baru"}
              </h3>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditForm({
                          ...editForm,
                          group_barang_id: e.target.value
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    ...
                    />
                    <label
                      htmlFor="is_inventory"
                      className="text-xs font-semibold text-slate-700 cursor-pointer"
                    >
                      Status Inventory (Lacak Stok)
                    </label>
                  </div>
                </div>
                <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditForm({
                            ...editForm,
                            harga_jual_1: Number(e.target.value),
                          })
                        }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditForm({
                            ...editForm,
                            harga_jual_2: Number(e.target.value),
                          })
                        }
                        className={`${ic} text-right`}
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setEditForm({
                            ...editForm,
                            harga_jual_3: Number(e.target.value),
                          })
                        }
                        className={`${ic} text-right`}
                      />
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditForm({
                          ...editForm,
                          perk_penjualan_id: e.target.value
                            ? Number(e.target.value)
                            : null,
                        })
                      }
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-sm p-3 mt-2">
                    ...
                    <p className="text-xs text-slate-600">
                      Harga Jual 1 adalah harga standar. Harga 2 & 3 untuk level
            ...
            <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex justify-end gap-3">
              ...
              <button
                onClick={handleSave}
                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-blue-700 hover:bg-blue-700 transition-colors flex items-center gap-2"
              >
                <Save size={16} /> Simpan
              </button>
            </div>
