...
const SetupTandaTangan: React.FC = () => {
      ...
      if (sigCanvas.current) {
        ...
        if (item.ttd_image) {
          ...
          sigCanvas.current.fromDataURL(
            `data:image/png;base64,${item.ttd_image}`,
          );
        }
      }
    }, 100);
  ...
  const inputClass =
    "w-full px-3 py-2 bg-white border border-slate-300 focus:outline-none focus:ring-2 focus:ring-slate-500/20 focus:border-slate-500 text-sm transition-colors rounded-sm";
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center">
        <div>
          <h2 className="text-lg font-semibold text-white">
            Setup Tanda Tangan Dokumen
          </h2>
          <p className="text-xs text-slate-300 mt-1">
            Kelola nama dan digital signature untuk berbagai dokumen.
          </p>
        </div>
        <button
              ...
              <tbody className="text-sm text-slate-700 divide-y divide-slate-100">
                    ...
                    <td className="px-4 py-3 text-center">
                      {item.ttd_image ? (
                        <div className="h-8 border border-slate-200 bg-white rounded overflow-hidden flex justify-center items-center">
                          <img
                            src={`data:image/png;base64,${item.ttd_image}`}
                            alt="Signature"
                ...
                {list.length === 0 && (
                  <tr>
                    ...
                    >
                      Belum ada data setup tanda tangan. Klik "Tambah Baru"
                      untuk memulai.
                    </td>
                  </tr>
                )}
            ...
            <div className="p-6 overflow-y-auto flex-1 flex flex-col gap-5">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 mt-1 italic">
                  ...
                  digital. Ini akan dicetak di dokumen terkait.
                </p>
              </div>
            </div>
