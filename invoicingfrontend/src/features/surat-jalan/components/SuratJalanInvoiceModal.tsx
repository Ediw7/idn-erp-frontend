      ...
      <div className="bg-white w-full max-w-lg rounded-md shadow-2xl flex flex-col overflow-hidden border border-slate-300">
        ...
        <div className="p-6 bg-slate-50 flex flex-col gap-4">
          <div className="bg-green-50 text-green-800 text-xs p-3 rounded-sm border border-green-200 font-medium">
            Proses ini akan mengonversi Surat Jalan saat ini menjadi Draft
            Invoice.
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              className={`${inputClass} bg-slate-100 h-16 resize-none cursor-not-allowed`}
              disabled
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              ...
              <select
                className={`${inputClass} bg-slate-100 font-mono cursor-not-allowed`}
                disabled
                value={form.no_sj || ""}
              >
        ...
        <div className="bg-white px-6 py-4 border-t border-slate-200 flex justify-end gap-3 shrink-0">
          ...
          <button
            onClick={() => {
              ...
              const pelangganParam = encodeURIComponent(
                form.pelanggan_id || "",
              );
              navigate(
                `/invoice?sj=${sjParam}&so=${soParam}&pelanggan=${pelangganParam}`,
              );
            }}
