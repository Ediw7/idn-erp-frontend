        ...
        <div className="flex-1 flex flex-col gap-3">
          <div className="flex items-start">
            ...
            <div className="flex gap-1 flex-1">
              <input
                type="text"
                className={`${inputClass} font-mono w-48 bg-slate-50`}
                readOnly
                value={form.no_sj || ""}
              />
          ...
          <div className="flex items-start">
            ...
            <div className="flex gap-1 flex-1">
              <select
                ...
                onChange={(e) =>
                  handlePelangganChange(form, setForm, e.target.value)
                }
              >
              ...
              <button
                onClick={() => navigate("/setup/pelanggan")}
                className="px-3 py-1.5 font-bold border border-slate-300 bg-slate-100 hover:bg-slate-200 rounded-sm text-slate-600"
              >
                +
              </button>
            </div>
        ...
        <div className="flex-1 flex flex-col gap-3">
          <div className="flex items-start">
            ...
            <div className="flex gap-1 flex-1">
              <select
                ...
                value={form.no_so || ""}
                onChange={(e) => handleSOChange(form, setForm, e.target.value)}
              >
                <option value="">-- Pilih --</option>
                {salesOrders.map((so) => (
          ...
          <div className="flex items-start">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, no_kendaraan: e.target.value })
              }
            />
          </div>
          <div className="flex items-start">
            ...
            <input
              type="text"
              className={`${inputClass} w-64 bg-slate-50 cursor-not-allowed`}
              readOnly
              value={form.no_invoice || ""}
              placeholder="Terisi jika sudah di-invoice"
            />
          </div>
          <div className="flex items-start">
          ...
          <div className="border border-slate-300 p-3 relative mt-2 rounded-sm bg-white shadow-sm">
            ...
            <div className="flex gap-2 mt-2">
              ...
              <input
                ...
                readOnly
                value={form.create_uid_name || user?.name || "Unknown"}
              />
            </div>
          </div>
          <div className="border border-slate-300 p-3 relative mt-3 rounded-sm bg-white shadow-sm">
            ...
            <div className="flex gap-2 mt-2">
              ...
              <input
                ...
                className="w-20 px-2 py-1.5 text-xs border border-slate-200 bg-slate-50 text-center rounded-sm"
                readOnly
                value={form.write_uid_name || user?.name || "Unknown"}
              />
            </div>
          </div>
        </div>
