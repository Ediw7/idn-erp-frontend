...
export const SuratJalanLineModal: React.FC<SuratJalanLineModalProps> = ({
  editLineIndex, lineForm, setLineForm, items, onClose, handleSaveLine
}) => {
  return (
        ...
        <div className="p-6 bg-slate-50 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" min="0" className="w-full px-3 py-2 bg-white border border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-500 rounded-sm text-sm text-right font-mono text-blue-700 font-bold" value={lineForm.kuantum === 0 ? '' : lineForm.kuantum} onChange={e => setLineForm({ ...lineForm, kuantum: Number(e.target.value) })} />
            </div>
          </div>
