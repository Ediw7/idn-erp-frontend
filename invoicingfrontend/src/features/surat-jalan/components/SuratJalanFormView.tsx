    ...
    <div className="bg-slate-50 shadow-sm border border-slate-300 flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <FilePlus size={14} /> BUAT SURAT JALAN{" "}
          </button>
          <button
            onClick={() => {
              const reportName =
          ...
          <button
            ...
            onClick={() => {
              if (!form.no_sj) {
                toast.error(
                  "Harap buat atau pilih Surat Jalan terlebih dahulu! Data invoice akan ditarik otomatis dari Surat Jalan tersebut.",
                );
                return;
              }
            ...
            title={
              form.no_invoice
                ? `Sudah di-invoice (${form.no_invoice})`
                : "Buat Invoice Baru dari SJ ini"
            }
          >
        ...
        <div className="flex gap-12">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-sm font-bold text-slate-800">
              {pelanggans.find((p) => String(p.id) === form.pelanggan_id)
                ?.nama || "-"}
            </span>
          </div>
          <div className="flex flex-col">
      ...
      <div className="bg-white border-t border-slate-200 p-4 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-20">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <button
            ...
            className="px-10 py-3.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 transition-all shadow-md hover:shadow-lg rounded-sm flex items-center gap-2 disabled:bg-slate-400"
          >
            {isSaving ? (
