...
const angkaMenjadiTerbilang = (angka: number): string => {
  ...
  if (angka < 100)
    return (
      angkaMenjadiTerbilang(Math.floor(angka / 10)) +
      " Puluh " +
      angkaMenjadiTerbilang(angka % 10)
    );
  if (angka < 200) return "Seratus " + angkaMenjadiTerbilang(angka - 100);
  if (angka < 1000)
    return (
      angkaMenjadiTerbilang(Math.floor(angka / 100)) +
      " Ratus " +
      angkaMenjadiTerbilang(angka % 100)
    );
  if (angka < 2000) return "Seribu " + angkaMenjadiTerbilang(angka - 1000);
  if (angka < 1000000)
    return (
      angkaMenjadiTerbilang(Math.floor(angka / 1000)) +
      " Ribu " +
      angkaMenjadiTerbilang(angka % 1000)
    );
  if (angka < 1000000000)
    return (
      angkaMenjadiTerbilang(Math.floor(angka / 1000000)) +
      " Juta " +
      angkaMenjadiTerbilang(angka % 1000000)
    );
  if (angka < 1000000000000)
    return (
      angkaMenjadiTerbilang(Math.floor(angka / 1000000000)) +
      " Miliar " +
      angkaMenjadiTerbilang(angka % 1000000000)
    );
  if (angka < 1000000000000000)
    return (
      angkaMenjadiTerbilang(Math.floor(angka / 1000000000000)) +
      " Triliun " +
      angkaMenjadiTerbilang(angka % 1000000000000)
    );
  return "";
};
...
export const useKwitansiLogic = () => {
  ...
  useEffect(() => {
    ...
    if (location.state && location.state.no_invoice) {
      const { no_invoice, pembeli_id, alamat, jumlah, keterangan } =
        location.state;
      const fetchAutoNo = async () => {
        ...
        setForm((prev: any) => ({
          ...
          untuk_pembayaran:
            keterangan || `Pembayaran Invoice No. ${no_invoice}`,
        }));
      };
      fetchAutoNo();
      setViewMode("form");
  ...
  const filteredData = useMemo(() => {
    return dataList.filter((k) => {
      if (filter.periode && !k.tgl_kwitansi?.startsWith(filter.periode))
        return false;
      if (
        filter.pelanggan_id &&
        String(k.pembeli_id) !== String(filter.pelanggan_id)
      )
        return false;
