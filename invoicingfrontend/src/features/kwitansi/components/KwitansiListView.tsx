          ...
          <div className="flex items-center gap-4 mt-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setFilter({ ...filter, pelanggan_id: e.target.value })
                }
              ...
              {filteredData.length > 0 ? (
                filteredData.map((item, idx) => {
                  const p = pelanggans.find(
                    (x) => String(x.id) === String(item.pembeli_id),
                  );
                  return (
                    <tr
                      key={idx}
                    ...
                    >
                      ...
                      <td className="px-3 py-2 font-mono text-slate-800 text-right">
                        {Number(item.jumlah || 0).toLocaleString("en-US", {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      <td
                        className="px-3 py-2 text-slate-600 truncate max-w-[200px]"
                        title={item.untuk_pembayaran}
                      >
                        {item.untuk_pembayaran || "-"}
                      </td>
