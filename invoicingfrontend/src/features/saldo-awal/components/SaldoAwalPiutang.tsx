...
const SaldoAwalPiutang: React.FC = () => {
  ...
  const [editForm, setEditForm] = useState<
    Omit<Partial<SaldoAwal>, "saldo_invoice"> & { saldo_invoice?: number | "" }
  >({
    no_invoice: "",
    tanggal: "",
  ...
  const [migrationFolder, setMigrationFolder] = useState(
    "C:\\krishand\\fp\\2007\\fpdat.mdb",
  );
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [resSaldo, resPelanggan, resProyek, resMataUang] =
        await Promise.all([
  ...
  const handleSave = async () => {
    ...
    try {
      const endpoint = editForm.id
        ? "/api/saldo-awal-piutang/update"
        : "/api/saldo-awal-piutang/create";
      const res = await axiosClient.post(endpoint, editForm);
      if (res.data.status === "success") {
        setIsFormOpen(false);
        fetchData();
        toast.success(
          editForm.id ? "Data berhasil diperbarui!" : "Data berhasil disimpan!",
        );
      } else {
  ...
  const handleTogglePaid = async (row: SaldoAwal) => {
    const action = row.is_paid
      ? "membatalkan status lunas"
      : "menandai sebagai lunas";
    const isConfirmed = await confirm(
      `Apakah Anda yakin ingin ${action} untuk invoice ${row.no_invoice}?`,
    );
    if (!isConfirmed) return;
  ...
  const handleMigration = async () => {
    ...
    setTimeout(() => {
      ...
      toast.success(
        "Simulasi migrasi selesai (Data dummy). Pada implementasi nyata, file ini akan diunggah ke server.",
      );
      fetchData();
    }, 1500);
  };
  ...
  const totalSaldo = filteredData
    .filter((item) => !item.is_paid)
    .reduce((sum, item) => sum + Number(item.saldo_invoice || 0), 0);
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">
            Kelola daftar saldo awal piutang pelanggan dan migrasi data.
          </p>
        </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col bg-white">
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-slate-700 bg-slate-100 border-b border-slate-300">
              <tr>
                ...
                <th className="px-3 py-2 border-r border-slate-300 font-semibold text-right">
                  Saldo Invoice
                </th>
                <th className="px-3 py-2 border-r border-slate-300 text-center font-semibold">
                  Paid
                </th>
                <th className="px-3 py-2 w-10 text-center font-semibold">
                  Aksi
                </th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={10} className="p-4 text-center text-slate-500">
                    Tidak ada data saldo awal piutang
                  </td>
                </tr>
              ) : (
                filteredData.map((row) => (
                  ...
                  >
                    ...
                    <td
                      className="px-3 py-1.5 border-r border-slate-200 truncate max-w-xs"
                      title={row.alamat}
                    >
                      {row.alamat}
                    </td>
                      ...
                      {row.tgl_jt && (
                        <div className="flex flex-col gap-0.5">
                          ...
                          {!row.is_paid &&
                            new Date(row.tgl_jt) <
                              new Date(new Date().setHours(0, 0, 0, 0)) && (
                              <span className="w-min px-1.5 py-[1px] rounded text-[9px] font-bold bg-red-100 text-red-700 uppercase tracking-wider">
                                Overdue
                              </span>
                            )}
                        </div>
                    ...
                    <td className="px-3 py-1.5 border-r border-slate-200 text-right">
                      {row.saldo_invoice.toLocaleString("en-US", {
        ...
        <div className="bg-slate-50 border-t border-slate-200 p-4 flex justify-end shrink-0">
          <div className="flex items-center border border-slate-300 bg-white rounded-sm overflow-hidden shadow-sm">
            <span className="px-4 py-2 text-xs font-bold text-slate-700 bg-slate-100 border-r border-slate-300 uppercase tracking-wide">
              Total
            </span>
            <span className="px-3 py-2 text-xs font-bold text-slate-600 bg-slate-50 border-r border-slate-300">
              IDR
            </span>
            <span className="px-5 py-2 text-sm font-bold text-slate-900 text-right min-w-[160px]">
              {Number(totalSaldo).toLocaleString("en-US", {
                minimumFractionDigits: 2,
              })}
            </span>
          </div>
            ...
            <div className="p-6 overflow-y-auto bg-slate-50 flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    value={editForm.no_invoice}
                    onChange={(e) =>
                      setEditForm({ ...editForm, no_invoice: e.target.value })
                    }
              ...
              <div>
                ...
                <select
                  value={editForm.pelanggan_id || ""}
                  onChange={(e) =>
                    setEditForm({
                      ...editForm,
                      pelanggan_id: Number(e.target.value),
                    })
                  }
                  className="w-full px-3 py-2 bg-white border border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-500 rounded-sm text-sm"
                >
                ...
                {editForm.pelanggan_id && (
                  <div className="mt-2 px-3 py-2 bg-slate-100/80 border border-slate-200 rounded-sm text-xs text-slate-600 leading-relaxed">
                    ...
                    {pelanggans.find((p) => p.id === editForm.pelanggan_id)
                      ?.alamat || (
                      <em className="text-slate-400">Tidak ada data alamat</em>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={editForm.tgl_jt}
                    onChange={(e) =>
                      setEditForm({ ...editForm, tgl_jt: e.target.value })
                    }
                    className="w-full px-3 py-2 bg-white border border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-500 rounded-sm text-sm"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    value={editForm.mata_uang_id || ""}
                    onChange={(e) =>
                      setEditForm({
                        ...editForm,
                        mata_uang_id: Number(e.target.value),
                      })
                    }
                    className="w-full px-3 py-2 bg-white border border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-500 rounded-sm text-sm"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white border border-slate-300 focus:outline-none focus:ring-1 focus:ring-slate-500 rounded-sm text-sm text-right"
                  />
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/20">
          <div className="bg-white rounded-md shadow-2xl border border-slate-300 w-[500px] overflow-hidden">
            <div className="px-4 py-3 bg-slate-800 text-white flex justify-between items-center border-b border-slate-700">
              ...
              <button onClick={() => setIsMigrationModalOpen(false)}>
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-2 border-b border-slate-200 bg-slate-50 flex gap-2">
              <button
                ...
                className="px-4 py-1.5 bg-white border border-slate-300 shadow-sm text-sm font-medium rounded-sm hover:bg-slate-100"
              >
                {" "}
                BATAL{" "}
              </button>
            ...
            <div className="p-6">
              <p className="text-sm text-slate-700 mb-6 font-medium leading-relaxed">
                Proses Migrasi Data akan mengimpor data Wajib Pajak dari program
                Krishand Faktur Pajak.
              </p>

              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-1">
                    Input dengan lengkap Nama Folder & Nama File Data Krishand
                    Faktur Pajak
                  </label>
                  <div className="flex gap-2">
                    ...
                    />
                    <button className="px-3 border border-slate-300 bg-slate-100 hover:bg-slate-200 rounded-sm font-bold text-slate-600">
                      ...
                    </button>
                  </div>
                  <p className="text-[10px] text-slate-400 mt-1">
                    *Pada web app, tombol ini akan membuka dialog unggah file
                    komputer Anda (.mdb/.csv)
                  </p>
                </div>
