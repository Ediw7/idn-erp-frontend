...
export const PembayaranFormView: React.FC<PembayaranFormViewProps> = ({
  ...
  const totalPembayaran = (form.lines || []).reduce(
    (acc: number, line: any) => acc + (Number(line.pembayaran) || 0),
    0,
  );
  const totalPotongan = (form.lines || []).reduce(
    (acc: number, line: any) => acc + (Number(line.potongan) || 0),
    0,
  );
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() =>
              navigate(
                form.no_bukti
                  ? `/laporan?pembayaran_number=${encodeURIComponent(form.no_bukti)}&reportName=${encodeURIComponent("Bukti Penerimaan Kas/Bank (1/2 Kwarto)")}`
                  : "/laporan?reportName=" +
                      encodeURIComponent(
                        "Bukti Penerimaan Kas/Bank (1/2 Kwarto)",
                      ),
              )
            }
              ...
              <div className="flex flex-col gap-2 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    value={totalPotongan.toLocaleString("en-US", {
                ...
                <div className="flex justify-end mt-2">
                  <div className="flex items-center border border-slate-300 bg-white rounded-sm overflow-hidden shadow-sm w-full">
                    ...
                    <span className="px-4 py-3 text-base font-bold text-slate-900 text-right min-w-[160px] font-mono">
                      {totalPembayaran.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
