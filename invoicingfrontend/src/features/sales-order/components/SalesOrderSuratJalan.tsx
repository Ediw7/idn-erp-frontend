...
export const SalesOrderSuratJalan: React.FC<SalesOrderSuratJalanProps> = ({
  form,
  gudangs,
}) => {
  return (
      ...
      <tbody className="divide-y divide-slate-200 bg-slate-50">
        {form.surat_jalans && form.surat_jalans.length > 0 ? (
          form.surat_jalans.map((sj: any, idx: number) => (
            <tr key={idx} className="hover:bg-blue-50 transition-colors">
              ...
              <td className="px-4 py-2 border-r border-slate-200">
                {sj.gudang_id
                  ? gudangs.find((g) => String(g.id) === String(sj.gudang_id))
                      ?.nama_gudang
                  : "-"}
              </td>
              <td className="px-4 py-2 border-r border-slate-200">
          ...
          <tr>
            ...
            >
              Belum ada riwayat pengiriman Surat Jalan untuk Sales Order ini.
            </td>
          </tr>
        )}
      </tbody>
