...
const KartuPiutang: React.FC = () => {
  ...
  const totalKalkulasi = useMemo(() => {
    const totalDebet = riwayatList.reduce((sum, item) => sum + (item.debet || 0), 0);
    const totalKredit = riwayatList.reduce((sum, item) => sum + (item.kredit || 0), 0);
  ...
  return (
    <div className="w-full h-full bg-white rounded-none flex flex-col shadow-sm border border-slate-300">
        ...
        <div className="p-6 bg-slate-900 w-full rounded-none flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h2 className="text-xl font-bold text-white tracking-wide">Kartu Piutang</h2>
            <p className="text-sm text-slate-300 mt-1">Riwayat Transaksi dan Saldo Piutang Pelanggan</p>
          </div>
        ...
        <div className="p-6 bg-white border-b border-slate-200 w-full rounded-none">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="col-span-1 lg:col-span-2">
              <label className="block text-sm font-semibold text-slate-700 mb-2">Pilih Nama Pelanggan</label>
              <select 
                ...
                onChange={e => setFilter({...filter, pelanggan_id: e.target.value ? Number(e.target.value) : ''})}
              >
            ...
            <div className="col-span-1">
              <label className="block text-sm font-semibold text-slate-700 mb-2">Pilih Mata Uang</label>
              <select 
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 text-gray-700 text-xs uppercase font-semibold border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                <tr>
                  ...
                  <th className="px-5 py-4 text-right whitespace-nowrap">Debet</th>
                  <th className="px-5 py-4 text-right whitespace-nowrap">Kredit</th>
                </tr>
              </thead>
                ...
                {isSearching ? (
                  <tr>
                    <td colSpan={6} className="px-5 py-12 text-center text-slate-400">
                      <div className="flex flex-col items-center justify-center gap-3">
                        ...
                        <span className="font-medium text-sm">Menarik riwayat data...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  riwayatList.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-50 transition-colors cursor-pointer group">
                      <td className="px-5 py-3 whitespace-nowrap">{row.tanggal}</td>
                      <td className="px-5 py-3 font-mono text-xs">{row.no_invoice}</td>
                      <td className="px-5 py-3 font-mono text-xs">{row.no_ref}</td>
                    ...
                    <td colSpan={6} className="px-5 py-16 text-center text-slate-400">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <span className="font-medium text-sm">
                          {filter.pelanggan_id === '' ? 'Pilih pelanggan untuk melihat kartu piutang.' : 'Tidak ada riwayat transaksi untuk pelanggan ini.'}
                        </span>
                      </div>
                    </td>
                  </tr>
        ...
        <div className="bg-slate-100 p-6 border-t border-slate-200 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
          ...
          <div className="flex flex-wrap items-center gap-4 w-full xl:w-auto">
            <div className="flex-1 min-w-[150px]">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Debet</label>
              <div className="bg-gray-50 border border-slate-300 rounded-sm px-4 py-2">
                <span className="text-lg font-bold text-slate-900 block text-right">{formatCurrency(totalKalkulasi.totalDebet)}</span>
              </div>
            </div>
            ...
            <div className="flex-1 min-w-[150px]">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Total Kredit</label>
              <div className="bg-gray-50 border border-slate-300 rounded-sm px-4 py-2">
                <span className="text-lg font-bold text-slate-900 block text-right">{formatCurrency(totalKalkulasi.totalKredit)}</span>
              </div>
            </div>
            ...
            <div className="flex-1 min-w-[180px]">
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Saldo</label>
              <div className="bg-blue-50 border border-blue-200 rounded-sm px-4 py-2 shadow-inner">
                <span className={`text-xl font-black block text-right ${totalKalkulasi.saldo < 0 ? 'text-red-600' : 'text-blue-700'}`}>
