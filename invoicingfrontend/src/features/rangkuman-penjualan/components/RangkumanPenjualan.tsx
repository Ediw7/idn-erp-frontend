          ...
          <thead className="bg-slate-50 sticky top-0 border-b border-slate-200">
            <tr>
              ...
              {columns.map((col: string, idx: number) => (
                <th key={idx} className={`px-2 py-1.5 border-r border-slate-200 font-semibold text-slate-700 ${idx === alignRightIdx ? 'text-right' : ''}`}>
                  {col}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {data.map((row: any, i: number) => (
              <tr key={i} className="border-b border-slate-100 hover:bg-blue-50">
                <td className="px-2 py-1.5 border-r border-slate-100 text-center text-slate-400">{i === 0 ? '▶' : ''}</td>
                {dataKeys.map((key: string, idx: number) => (
                  <td key={idx} className={`px-2 py-1.5 border-r border-slate-100 ${idx === alignRightIdx ? 'text-right font-medium' : ''}`}>
                    {row[key]}
                  </td>
                ))}
              </tr>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden">
      ...
      <div className="bg-slate-800 px-6 py-5 flex justify-between items-start shrink-0">
        <div>
          <h1 className="text-xl font-bold text-white tracking-wide">Rangkuman Data Penjualan</h1>
          <p className="text-slate-400 text-sm mt-1">Dashboard ringkasan statistik performa penjualan barang dan nilai piutang pelanggan.</p>
        </div>
