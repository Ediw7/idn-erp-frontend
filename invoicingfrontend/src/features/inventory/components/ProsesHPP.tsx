...
const ProsesHPP: React.FC = () => {
  ...
  const handleProses = async () => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      toast.success("Proses perhitungan HPP selesai dilakukan!", {
        id: toastId,
      });
      handleBatal();
    }, 2000);
  ...
  const inputClass =
    "w-full md:w-1/3 px-3 py-2 border border-slate-300 bg-white rounded-sm text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all disabled:opacity-50 disabled:bg-gray-100";
      ...
      <div className="p-6 bg-slate-900 w-full rounded-none flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1">
            Modul utilitas untuk menghitung ulang Harga Pokok Penjualan (HPP)
            Inventory
          </p>
        </div>
      ...
      <div className="p-6 bg-white w-full rounded-none">
        ...
        <div className="flex items-start gap-3 bg-blue-50 text-blue-800 p-4 rounded-md mb-8 border border-blue-100">
          ...
          <p className="text-sm font-medium leading-relaxed">
            Proses ini akan menghitung HPP atas semua transaksi yang belum
            diproses atau menghitung ulang HPP jika ditemukan data yang tidak
            sesuai. Proses ini dapat dilakukan setiap saat.
          </p>
        </div>
          ...
          <div className="space-y-4 pt-2">
            <label className="flex items-center gap-3 cursor-pointer group w-fit">
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, prosesDariAwal: e.target.checked })
                }
                disabled={isLoading}
              />
            ...
            <label className="flex items-center gap-3 cursor-pointer group w-fit">
              <input
                ...
                onChange={(e) => {
                  ...
                  setForm({
                    ...form,
                    prosesSemuaBarang: checked,
                    item_id: checked ? "" : form.item_id,
                  });
                }}
                disabled={isLoading}
              />
