            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 text-gray-700 text-xs uppercase font-semibold border-b border-slate-200 sticky top-0 shadow-sm z-10">
                <tr>
                  ...
                  <th className="px-5 py-4 text-right whitespace-nowrap">Saldo Gudang</th>
                  <th className="px-5 py-4 text-right whitespace-nowrap">Blm Dikirim (SO)</th>
                  <th className="px-5 py-4 text-right whitespace-nowrap">Saldo Setelah SO</th>
                </tr>
              </thead>
                ...
                {isLoading ? (
                  <tr>
                    <td colSpan={7} className="px-5 py-12 text-center">
                      <div className="flex flex-col items-center justify-center text-slate-400 gap-3">
                        ...
                        <span className="font-medium text-sm">Memuat rekapan stok...</span>
                      </div>
                    </td>
                  </tr>
                ) : rekapList.length > 0 ? (
                  rekapList.map((row) => (
                    <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-5 py-3 font-mono text-xs text-slate-600">{row.kode_barang}</td>
                      <td className="px-5 py-3 font-medium text-slate-900">{row.nama_barang}</td>
                      <td className="px-5 py-3 text-slate-600">{row.group_barang}</td>
                      ...
                      <td className="px-5 py-3 text-right font-medium text-blue-600">
                        {new Intl.NumberFormat('id-ID').format(row.blm_dikirim_so)}
                      </td>
                      <td className={`px-5 py-3 text-right font-bold ${row.saldo_setelah_so < 0 ? 'text-red-600' : 'text-slate-900'}`}>
                        {new Intl.NumberFormat('id-ID').format(row.saldo_setelah_so)}
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan={7} className="px-5 py-16 text-center text-slate-400">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <span className="font-medium text-sm">Tidak ada data rekap stok untuk filter ini.</span>
                      </div>
                    </td>
                  </tr>
                )}
