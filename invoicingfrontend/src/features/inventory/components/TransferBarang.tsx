...
const TransferBarang: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      try {
        ...
        if (defGudang && !form.gudang_asal_id) {
          setForm((prev) => ({ ...prev, gudang_asal_id: defGudang.id! }));
        }
      } catch (error: any) {
  ...
  const handleBaru = () => {
    ...
    setForm({
      no_bukti: "",
      tanggal: new Date().toISOString().split("T")[0],
      gudang_asal_id: defGudang ? defGudang.id! : "",
      gudang_tujuan_id: "",
      keterangan: "",
    });
    setDetails([
  ...
  const handleSimpan = async () => {
    if (
      form.gudang_asal_id === form.gudang_tujuan_id &&
      form.gudang_asal_id !== ""
    ) {
  ...
  const readOnlyClass =
    "w-full px-2 py-1.5 bg-gray-50 border border-slate-200 text-slate-500 rounded text-sm focus:outline-none";
      ...
      <div className="p-6 bg-slate-900 w-full rounded-none flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1">
            Form perpindahan stok barang antar gudang
          </p>
        </div>
      ...
      <div className="p-6 bg-white w-full rounded-none">
          ...
          <div className="lg:col-span-1">
            ...
            <select
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  gudang_asal_id: e.target.value ? Number(e.target.value) : "",
                })
              }
            >
              ...
              {gudangs.map((g) => (
                <option
                  ...
                  value={g.id}
                  disabled={
                    form.gudang_tujuan_id !== "" &&
                    g.id === form.gudang_tujuan_id
                  }
                >
                  {g.nama_gudang}
                </option>
          ...
          <div className="lg:col-span-1">
            ...
            <select
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  gudang_tujuan_id: e.target.value
            ...
            >
              ...
              {gudangs.map((g) => (
                <option
                  ...
                  value={g.id}
                  disabled={
                    form.gudang_asal_id !== "" && g.id === form.gudang_asal_id
                  }
                >
                  {g.nama_gudang}
                </option>
                ...
                >
                  <td className="p-2">
                    <select
                      ...
                      value={row.item_id}
                      onChange={(e) =>
                        handleDetailChange(row.id, "item_id", e.target.value)
                      }
                    >
                  ...
                  <td className="p-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleDetailChange(row.id, "keterangan", e.target.value)
                      }
                    />
                  </td>
        ...
        <div className="flex justify-end mt-4">
          <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg border border-slate-200 w-full md:w-1/3 shadow-sm">
            <label className="text-sm font-bold text-slate-700 flex-1">
              TOTAL QUANTITY
            </label>
            <div className="w-1/3">
              <input
