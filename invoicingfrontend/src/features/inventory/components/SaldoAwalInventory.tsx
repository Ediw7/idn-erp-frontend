...
const SaldoAwalInventory: React.FC = () => {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await axiosClient.post('/api/inventory/saldo-awal/get', {});
  ...
  const filteredData = dataList.filter(item => 
    (item.gudang_nama?.toLowerCase() || '').includes(searchQuery.toLowerCase()) ||
  ...
  const handleEdit = (record: SaldoAwalInventoryData) => {
    setFormData({
      ...record,
      lines: record.lines.length > 0 ? [...record.lines, createEmptyLine()] : [createEmptyLine()]
    });
    setIsFormOpen(true);
  };

  const handleDelete = async (id: number) => {
    const isConfirmed = await confirm('Apakah Anda yakin ingin menghapus data saldo awal ini?');
  ...
  const handleLineChange = (index: number, field: keyof SaldoAwalInventoryLine, value: any) => {
    const newLines = [...formData.lines];
    newLines[index] = { ...newLines[index], [field]: value };
    
    if (index === newLines.length - 1 && field === 'item_id' && value !== null) {
  ...
  return (
    <div className="bg-white shadow-sm border border-slate-300 flex flex-col h-[calc(100vh-8rem)] relative">
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
        <div>
          <h2 className="text-lg font-semibold text-white">Saldo Awal Persediaan</h2>
          <p className="text-xs text-slate-300 mt-1">Kelola data saldo awal barang di gudang.</p>
        </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-slate-700 bg-slate-100 border-b border-slate-300">
              <tr>
                ...
                <th className="px-4 py-2 border-r border-slate-300 font-semibold">Gudang</th>
                ...
                <th className="px-4 py-2 border-r border-slate-300 text-center font-semibold w-24">Jml Item</th>
                <th className="px-4 py-2 border-r border-slate-300 text-right font-semibold w-40">Total Saldo (IDR)</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={7} className="p-4 text-center text-slate-500">Tidak ada data saldo awal</td></tr>
              ) : (
                filteredData.map((row, index) => (
                  <tr key={row.id} className="border-b border-slate-200 hover:bg-blue-50 transition-colors">
                    ...
                    <td className="px-4 py-2 border-r border-slate-200 font-medium">{row.gudang_nama || '-'}</td>
                    <td className="px-4 py-2 border-r border-slate-200">{row.tanggal}</td>
                    <td className="px-4 py-2 border-r border-slate-200">{row.keterangan || '-'}</td>
                    <td className="px-4 py-2 border-r border-slate-200 text-center font-mono bg-slate-50">{row.lines.length}</td>
          ...
          <div className="bg-white border border-slate-300 shadow-2xl rounded-sm w-full max-w-5xl h-full max-h-[90vh] flex flex-col overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-2 gap-8 border-b-2 border-slate-300 shrink-0 bg-white">
                <div className="flex flex-col gap-3">
                  <div className="flex items-center">
                    <label className="text-xs font-semibold text-slate-700 w-24">Gudang</label>
                    <select 
                      value={formData.gudang_id || ''} 
                      onChange={e => setFormData({...formData, gudang_id: Number(e.target.value)})}
                      className="flex-1 px-2 py-1.5 bg-white border border-slate-300 focus:outline-none focus:border-blue-500 text-xs"
                    >
                      <option value="">-- Pilih Gudang --</option>
                      {gudangs.map(g => (
                        <option key={g.id} value={g.id}>{g.kode_gudang} - {g.nama_gudang}</option>
                      ))}
                    </select>
                  </div>
                  <div className="flex items-center">
                    <label className="text-xs font-semibold text-slate-700 w-24">Tanggal</label>
                    <input 
                      type="date" 
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-start">
                    <label className="text-xs font-semibold text-slate-700 w-24 pt-1">Keterangan</label>
                    <textarea 
              ...
              <div className="flex-1 overflow-auto bg-white p-4">
                <table className="w-full text-left border-collapse whitespace-nowrap min-w-max border border-slate-300">
                  <thead className="bg-slate-200 text-slate-800 sticky top-0 z-10 shadow-sm border-b border-slate-400">
                    <tr>
                      <th className="w-10 border-r border-slate-300 py-2 text-center font-bold text-xs">No</th>
                      <th className="w-48 border-r border-slate-300 px-3 py-2 font-bold text-xs">Kode Barang</th>
                      <th className="w-80 border-r border-slate-300 px-3 py-2 font-bold text-xs">Nama Barang</th>
                      <th className="w-20 border-r border-slate-300 px-3 py-2 text-center font-bold text-xs">Satuan</th>
                      <th className="w-32 border-r border-slate-300 px-3 py-2 text-right font-bold text-xs">Quantity</th>
                      <th className="w-32 border-r border-slate-300 px-3 py-2 text-right font-bold text-xs">HPP</th>
                      <th className="w-40 border-r border-slate-300 px-3 py-2 text-right font-bold text-xs">Jumlah HPP</th>
                      <th className="w-12 py-2 text-center font-bold text-xs">Aksi</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200 text-xs">
                    {formData.lines.map((line, index) => (
                      <tr key={index} className="hover:bg-blue-50 focus-within:bg-blue-50">
                        <td className="w-10 border-r border-slate-300 text-center py-1.5 text-slate-500 bg-slate-100">{index + 1}</td>
                        ...
                        <td className="w-48 border-r border-slate-300 p-0 relative">
                          <div className="flex h-full w-full">
                            <input 
                              ...
                              onChange={(e) => handleLineChange(index, 'item_kode', e.target.value)}
                              className="flex-1 w-full px-2 py-1.5 outline-none bg-transparent"
                              placeholder={index === formData.lines.length - 1 ? "Pilih barang..." : ""}
                            />
                            <button 
                              onClick={() => { setShowItemSearch(index); setItemSearchQuery(''); }}
                          ...
                          {showItemSearch === index && (
                            <div className="absolute left-0 top-full mt-1 w-96 bg-white border border-slate-400 shadow-xl z-50 rounded-sm">
                              <div className="p-2 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                                <input 
                                  type="text" autoFocus placeholder="Cari kode atau nama item..." 
                                  value={itemSearchQuery} onChange={e => setItemSearchQuery(e.target.value)}
                                  className="w-full px-2 py-1.5 text-xs border border-slate-300 focus:outline-none focus:border-blue-500"
                                />
                                <button onClick={() => setShowItemSearch(null)} className="ml-2 text-slate-500 hover:text-red-500">X</button>
                              </div>
                              <div className="max-h-48 overflow-y-auto">
                                {items.filter(i => 
                                  i.kode.toLowerCase().includes(itemSearchQuery.toLowerCase()) || 
                                  i.nama.toLowerCase().includes(itemSearchQuery.toLowerCase())
                                ).map(item => (
                                  <div 
                                    key={item.id} onClick={() => selectItem(index, item)}
                                  ...
                                  >
                                    ...
                                    <span className="text-slate-600 truncate max-w-[200px]">{item.nama}</span>
                                  </div>
                                ))}
                              </div>
                            </div>
                        ...
                        <td className="w-80 border-r border-slate-300 px-3 py-1.5 bg-slate-50 text-slate-600 truncate" title={line.item_nama}>
                          {line.item_nama}
                        </td>
                        ...
                        <td className="w-32 border-r border-slate-300 p-0">
                          <input 
                            type="number" value={line.quantity === 0 ? '' : line.quantity}
                        ...
                        <td className="w-32 border-r border-slate-300 p-0">
                          <input 
                            type="number" value={line.hpp === 0 ? '' : line.hpp}
                            onChange={(e) => handleLineChange(index, 'hpp', Number(e.target.value))}
                            className="w-full px-3 py-1.5 outline-none bg-transparent text-right"
                          />
                        </td>
                        <td className="w-40 border-r border-slate-300 px-3 py-1.5 text-right bg-slate-50 text-slate-800 font-bold">
                          {((line.quantity || 0) * (line.hpp || 0)).toLocaleString('id-ID', { minimumFractionDigits: 2 })}
                        </td>
                        <td className="w-12 text-center py-1.5">
                          {index !== formData.lines.length - 1 && (
                            <button onClick={() => removeLine(index)} className="text-red-500 hover:text-red-700 p-1">
                              <Trash2 size={12} />
                            </button>
                          )}
                        </td>
            ...
            <div className="bg-slate-100 border-t border-slate-300 px-6 py-4 flex items-center justify-between shrink-0">
              <div className="flex items-center border border-slate-300 bg-white shadow-sm">
                <span className="px-4 py-1.5 text-xs font-bold text-slate-700 bg-slate-200 border-r border-slate-300">TOTAL KESELURUHAN (IDR)</span>
                <span className="px-6 py-1.5 text-sm font-bold text-slate-900">
                  {calcDocTotal(formData.lines).toLocaleString('id-ID', { minimumFractionDigits: 2 })}
                </span>
              </div>
