...
import {
  getSuratJalan,
  getInvoices,
  getPembayaran as getPembayaranTrx,
  getKwitansi,
...
const PreviewLaporan: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const targetNo =
      params.get("no_sj") ||
      params.get("no_so") ||
      params.get("no_invoice") ||
      params.get("no_pembayaran") ||
      params.get("kwitansi_number") ||
      "";

    let title = "LAPORAN";
    let targetFormulir = "";
    if (type.startsWith("bpk")) {
      title = "BUKTI PENERIMAAN KAS";
      targetFormulir = "Pembayaran";
    }
    if (type.startsWith("sj")) {
      title = "SURAT JALAN";
      targetFormulir = "Surat Jalan";
    }
    if (type.startsWith("so")) {
      title = "SALES ORDER";
      targetFormulir = "Sales Order";
    }
    if (type.startsWith("inv")) {
      title = "INVOICE";
      targetFormulir = "Invoice";
    }
    if (
      params.get("reportName")?.includes("Kwitansi") ||
      targetNo.startsWith("KT/")
    ) {
      title = "KWITANSI";
      targetFormulir = "Kwitansi";
    }
    ...
    const fetchData = async () => {
      try {
        ...
        if (tandaTanganData && tandaTanganData.length > 0) {
          const match = tandaTanganData.find(
            (t: any) =>
              t.jenis_formulir?.toLowerCase() === targetFormulir.toLowerCase(),
          );
          if (match) {
            setSignature({
              nama: match.nama,
              jabatan: match.jabatan,
              ttd_image: match.ttd_image,
            });
          } else {
            setSignature({
              nama: tandaTanganData[0].nama,
              jabatan: tandaTanganData[0].jabatan,
              ttd_image: tandaTanganData[0].ttd_image,
            });
          }
        }
        ...
        if (targetFormulir === "Sales Order") {
          ...
          if (so) {
            ...
            p = pelanggans.find(
              (x: any) => String(x.id) === String(so.pelanggan_id),
            );
            const pembNama =
              pembayarans.find(
                (x: any) => String(x.id) === String(so.pembayaran_id),
              )?.nama || "";
            const salesNama =
              salesmans.find(
                (x: any) => String(x.id) === String(so.salesman_id),
              )?.nama || "";
            setMeta({
            ...
            mappedLines = (so.lines || []).map((l: any) => {
              const item = items.find(
                (i: any) => String(i.id) === String(l.item_id),
              );
              return {
          ...
          if (sj) {
            ...
            p = pelanggans.find(
              (x: any) => String(x.id) === String(sj.pelanggan_id),
            );
            setMeta({
              no_so: sj.no_so,
              no_po: sj.no_po,
          ...
          if (inv) {
            ...
            p = pelanggans.find(
              (x: any) => String(x.id) === String(inv.pembeli_id),
            );
            const pembNama =
              pembayarans.find(
                (x: any) => String(x.id) === String(inv.cara_pembayaran),
              )?.nama ||
              inv.cara_pembayaran ||
              "";
            const salesNama =
              salesmans.find(
                (x: any) => String(x.id) === String(inv.salesman_id),
              )?.nama || "";
            setMeta({
          ...
          const pem = pembayaranTrxData.find(
            (i: any) => i.no_bukti === targetNo,
          );
          if (pem) {
            setDocDate(pem.tgl_pembayaran || pem.tanggal);
            p = pelanggans.find(
              (x: any) => String(x.id) === String(pem.pelanggan_id),
            );
            setMeta({
            ...
            (pem.lines || []).forEach((l: any) => {
              const inv = invData.find(
                (i: any) => i.no_invoice === l.no_invoice,
              );
              ...
              if (inv && inv.lines) {
                inv.lines.forEach((invL: any) => {
                  const item = items.find(
                    (i: any) => String(i.id) === String(invL.item_id),
                  );
                  tempLines.push({
          ...
          if (k) {
            ...
            p = pelanggans.find(
              (x: any) => String(x.id) === String(k.pembeli_id),
            );
            setMeta({
              jumlah: k.jumlah || 0,
            ...
            if (k.penandatangan) {
              setSignature((prev) =>
                prev
                  ? {
                      ...prev,
                      nama: k.penandatangan,
                      jabatan: k.jabatan || prev.jabatan,
                    }
                  : {
                      nama: k.penandatangan,
                      jabatan: k.jabatan || "",
  ...
  const totalAkhir =
    dpp + ppnAmount + pphAmount + ppnbmAmount + (meta.ongkos_angkut || 0);

  if (isLoading) {
    return (
      <div className="w-full h-screen flex items-center justify-center text-slate-500 font-semibold bg-slate-100">
        Memuat Dokumen...
      </div>
    );
  }
        ...
        <div className="w-full max-w-[210mm] min-h-[297mm] bg-white shadow-lg print:shadow-none p-10 flex flex-col relative">
          ...
          <div className="flex justify-between items-start border-b-[3px] border-blue-900 pb-5 mb-6">
            <div className="flex flex-col">
              <h2 className="text-2xl font-black text-blue-900 tracking-tight uppercase">
                {company.name}
              </h2>
              <p className="text-sm text-slate-600 mt-1 max-w-[300px] leading-relaxed">
                {company.alamat}
              </p>
            </div>
            <div className="text-right flex flex-col items-end">
              <h1 className="text-3xl font-black text-slate-300 uppercase tracking-widest">
                {reportTitle}
              </h1>
              <div className="mt-2 bg-blue-50 border border-blue-100 px-4 py-2 rounded-sm inline-flex flex-col items-end">
                ...
                <span className="text-xs text-slate-600 mt-0.5">
                  Tanggal: <span className="font-semibold">{docDate}</span>
                </span>
              </div>
            </div>
          </div>
                ...
                <table className="w-full text-base">
                  <tbody>
                    <tr className="border-b border-slate-100">
                      <td className="py-5 w-48 font-bold text-slate-700 align-top uppercase tracking-wider text-sm">
                        Telah Terima Dari
                      </td>
                      <td className="py-5 w-6 align-top font-bold text-slate-400">
                        :
                      </td>
                      <td className="py-5 text-slate-900 font-bold text-lg">
                        {customerName}
                      </td>
                    </tr>
                    <tr className="border-b border-slate-100">
                      <td className="py-5 font-bold text-slate-700 align-top uppercase tracking-wider text-sm">
                        Uang Sebesar
                      </td>
                      <td className="py-5 align-top font-bold text-slate-400">
                        :
                      </td>
                    ...
                    <tr>
                      ...
                      <td className="py-5 text-slate-800 whitespace-pre-wrap text-base font-medium">
                        {meta.untuk_pembayaran}
                      </td>
                    </tr>
                  </tbody>
                </table>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-100 border-y border-slate-300">
                    <th className="py-3 px-3 text-left w-12 font-bold text-slate-700">
                      No
                    </th>
                    <th className="py-3 px-3 text-left w-24 font-bold text-slate-700">
                      Kode
                    </th>
                    <th className="py-3 px-3 text-left font-bold text-slate-700">
                      Deskripsi Barang
                    </th>
                    <th className="py-3 px-3 text-center w-20 font-bold text-slate-700">
                      Satuan
                    </th>
                    <th className="py-3 px-3 text-center w-20 font-bold text-slate-700">
                      Qty
                    ...
                    {isFinancial && (
                      <>
                        <th className="py-3 px-3 text-right w-28 font-bold text-slate-700">
                          Harga ({meta.mata_uang || "IDR"})
                        </th>
                        <th className="py-3 px-3 text-right w-20 font-bold text-slate-700">
                          Disc
                        </th>
                        <th className="py-3 px-3 text-right w-32 font-bold text-slate-700">
                          Total
                        </th>
                      </>
                    )}
                    {!isFinancial && (
                      <th className="py-3 px-3 text-left w-48 font-bold text-slate-700">
                        Keterangan
                      </th>
                    )}
                  </tr>
                      ...
                      return lines.map((l, i) => {
                        ...
                        if (l.keterangan === "HEADER") {
                          return (
                            <tr
                              key={i}
                              className="bg-slate-100 border-y-2 border-slate-200"
                            >
                              <td
                                colSpan={isFinancial ? 8 : 6}
                                className="py-2.5 px-3 font-bold text-slate-800 text-xs uppercase tracking-wide"
                              >
                                {l.nama}
                              </td>
                            </tr>
                        ...
                        if (l.keterangan === "PAYMENT") {
                          return (
                            <tr
                              key={i}
                              className="bg-emerald-50/50 border-b-2 border-emerald-100 group"
                            >
                              <td
                                colSpan={5}
                                className="py-3 px-3 text-right font-bold text-emerald-800 text-xs uppercase tracking-wide"
                              >
                                {l.nama}
                              </td>
                              {isFinancial && (
                                <>
                                  <td className="py-3 px-3 text-right font-mono text-emerald-700 text-xs font-semibold">
                                    {fmt(l.harga_satuan)}
                                  </td>
                                  <td className="py-3 px-3 text-right text-emerald-600 text-xs">
                                    {l.disc_harga > 0 ? fmt(l.disc_harga) : "-"}
                                  </td>
                                  <td className="py-3 px-3 text-right font-mono font-bold text-emerald-900 text-xs bg-emerald-100/50">
                                    {fmt(jumlah)}
                                  </td>
                                </>
                              )}
                              {!isFinancial && (
                                <td className="py-3 px-3 text-emerald-600 text-xs">
                                  {l.keterangan}
                                </td>
                              )}
                            </tr>
                        ...
                        return (
                          <tr key={i} className="group">
                            ...
                            <td className="py-3 px-3 text-center font-bold text-slate-800 bg-slate-50/50 group-hover:bg-transparent">
                              {l.kuantum}
                            </td>
                            {isFinancial && (
                              <>
                                ...
                                <td className="py-3 px-3 text-right font-mono font-semibold text-slate-800 text-xs">
                                  {fmt(jumlah)}
                                </td>
                              </>
                            )}
            ...
            {reportType === "Kwitansi" ? (
              <div className="flex justify-end items-start text-sm text-slate-800 mt-4 mr-12">
                <div className="flex flex-col items-center w-64 text-center">
                  <span className="font-semibold text-slate-600 mb-2">
                    Yang Menerima,
                  </span>
                  <div className="mt-6 w-full flex flex-col items-center min-h-[70px]">
                    {signature?.ttd_image ? (
                      <img
                        src={`data:image/png;base64,${signature.ttd_image}`}
                        alt="Tanda Tangan"
                        className="h-20 object-contain mb-2 opacity-90 mix-blend-multiply"
                      />
                    ) : (
                      <div className="h-20" />
                    )}
