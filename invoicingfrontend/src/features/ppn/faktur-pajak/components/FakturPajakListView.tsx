          ...
          <div className="flex items-center gap-4 mt-1.5">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setFilterTglAkhir(
                    `${currentYear}-${e.target.value}-${lastDay}`,
                  );
                }}
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  setFilterTglAkhir(
                    `${e.target.value}-${currentMonth}-${lastDay}`,
                  );
                }}
                className="text-xs bg-slate-700 text-white border border-slate-600 rounded-sm px-2 py-0.5 outline-none focus:border-slate-400"
              >
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                value={filterNoFaktur}
                onChange={(e) => setFilterNoFaktur(e.target.value)}
              />
            </div>
            <div className="flex items-center gap-2">
              ...
              <input
                type="text"
                className="text-xs bg-slate-700 text-white border border-slate-600 rounded-sm px-2 py-0.5 outline-none focus:border-slate-400 w-28"
                value={filterNamaPembeli}
                onChange={(e) => setFilterNamaPembeli(e.target.value)}
              />
            </div>
          </div>
        </div>
              ...
              {loadingData ? (
                ...
                <tr>
                  <td
                    ...
                    className="text-center p-6 italic text-slate-500"
                  >
                    Tidak ada data Faktur Pajak. Klik Buka Form untuk membuat
                    data baru.
                  </td>
                </tr>
              ) : (
                dataList.map((item, idx) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right font-mono text-slate-800">
                      {(item.dpp_rp || item.dpp || 0).toLocaleString("en-US", {
                    ...
                    <td className="px-3 py-2 text-right font-mono text-slate-800">
                      {(item.ppn_rp || 0).toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                      })}
                    </td>
      ...
      <div className="bg-slate-50 border-t border-slate-200 px-6 py-3 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-4 bg-slate-100 px-4 py-2 rounded-sm border border-slate-200 shadow-sm">
          ...
          <span className="text-sm font-bold text-blue-700 font-mono">
            {dataList
              .reduce((acc, curr) => acc + (curr.ppn_rp || 0), 0)
