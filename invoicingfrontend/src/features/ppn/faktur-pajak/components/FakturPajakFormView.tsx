    ...
    <div className="bg-slate-50 shadow-sm border border-slate-300 flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setShowPenggantiModal(true)}
          ...
          <button
            onClick={() => setShowEFakturModal(true)}
            className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-white bg-slate-600 border border-transparent hover:bg-slate-500 transition-colors ml-2 rounded-sm shadow-sm"
          >
            e-FAKTUR
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        ...
        <div className="bg-white border-l-4 border-l-blue-600 border-y border-r border-slate-300 rounded-sm shadow-sm p-4 shrink-0 flex justify-between items-center">
          <div className="flex gap-12">
            ...
            <div>
              ...
              <span className="text-base font-bold text-slate-800">
                {pelanggans.find((p) => p.id === form.pembeli_id)?.nama || "-"}
              </span>
            </div>
            <div>
              <span className="block text-[10px] font-bold text-slate-500 uppercase">
          ...
          <div className="flex gap-3">
            <span className="px-3 py-1 bg-blue-50 text-blue-700 font-bold text-xs rounded-sm border border-blue-200 uppercase">
              {form.jenis_transaksi?.split(" - ")[0] || "01"}
            </span>
          </div>
        </div>
              ...
              <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-0">
                ...
                <div className="flex flex-col gap-3">
                  <div className="flex items-start">
                    ...
                    <div className="flex gap-2 flex-1">
                      ...
                      >
                        ...
                        {fakturPajakSetups.map((fp) => (
                          <option key={fp.id} value={fp.id}>
                            {fp.no_seri_awal} - {fp.no_seri_akhir}
                          </option>
                        ))}
                      </select>
                      <button
                        onClick={() => setShowNsfpModal(true)}
                        className="px-3 border border-slate-300 bg-slate-100 hover:bg-slate-200 text-xs font-bold rounded-sm transition-colors"
                      >
                        +
                      </button>
                    </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <textarea
                      className={`${inputClass} h-16 resize-none bg-slate-100 text-slate-500`}
                      readOnly
                      value={form.alamat || ""}
                    />
                  </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div className="flex gap-2 flex-1 items-center">
                      <input
                        type="text"
                        className={`${inputClass} font-mono ${form.jenis_status === "Normal" ? "bg-slate-100 text-slate-500" : ""}`}
                        disabled={form.jenis_status === "Normal"}
                        value={form.fp_diganti || ""}
                        onChange={(e) =>
                          setForm({ ...form, fp_diganti: e.target.value })
                        }
                        placeholder="No. FP Lama"
                      />
                      ...
                      <input
                        type="date"
                        className={`${inputClass} w-36 ${form.jenis_status === "Normal" ? "bg-slate-100 text-slate-500" : ""}`}
                        disabled={form.jenis_status === "Normal"}
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex items-start">
                    ...
                    >
                      ...
                      {invoices
                        .filter((inv) =>
                          form.pembeli_id
                            ? Number(inv.pelanggan_id) ===
                              Number(form.pembeli_id)
                            : true,
                        )
                        .map((inv) => (
                  ...
                  <div className="flex items-start">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setForm({ ...form, tarif_ppn: Number(e.target.value) })
                      }
                    />
                  </div>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="text-xs text-slate-700 bg-slate-100 border-b border-slate-300">
                    <tr>
                      ...
                      <th className="px-3 py-2 w-16 text-center font-semibold">
                        Aksi
                      </th>
                    </tr>
                  </thead>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-1.5 border-r border-slate-200">
                            {itemInfo
                              ? `${itemInfo.kode} - ${itemInfo.nama}`
                              : line.kode_barang
                                ? `${line.kode_barang} - ${line.nama_barang}`
                                : "-"}
                          </td>
              ...
              <div className="flex flex-col gap-2 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <input
                    type="text"
                    readOnly
                    className="w-48 px-3 py-1.5 text-right bg-slate-100 border border-slate-300 font-mono text-sm font-bold rounded-sm"
                    value={totals.totalHargaJual.toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                    })}
                  />
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <input
                    type="number"
                    className="w-48 px-3 py-1.5 text-right bg-white border border-slate-300 focus:outline-none focus:border-blue-500 font-mono text-sm rounded-sm"
                    value={form.potongan || ""}
                    onChange={(e) =>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm({ ...form, uang_muka: Number(e.target.value) })
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <input
                    type="text"
                    readOnly
                    className="w-48 px-3 py-1.5 text-right bg-slate-100 border border-slate-300 font-mono text-sm rounded-sm"
                    value={totals.dppValas.toLocaleString("en-US", {
                      minimumFractionDigits: 2,
                    })}
                  />
                </div>
                ...
                <div className="flex justify-end mt-2">
                  <div className="flex items-center border border-slate-300 bg-white rounded-sm overflow-hidden shadow-sm w-full">
                    <span className="px-4 py-3 text-xs font-bold text-slate-700 bg-slate-100 border-r border-slate-300 uppercase tracking-wide flex-1">
                      Dasar Pengenaan Pajak
                    </span>
                    <span className="px-3 py-3 text-xs font-bold text-slate-600 bg-slate-50 border-r border-slate-300">
                      IDR
                    </span>
                    <span className="px-4 py-3 text-base font-bold text-slate-900 text-right min-w-[160px] font-mono">
                      {totals.dpp.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
                ...
                <div className="flex justify-end mt-1">
                  <div className="flex items-center border border-blue-300 bg-blue-50 rounded-sm overflow-hidden shadow-sm w-full">
                    <span className="px-4 py-3 text-xs font-bold text-blue-800 bg-blue-100 border-r border-blue-200 flex-1">
                      PPN = {form.tarif_ppn}% x DPP
                    </span>
                    <span className="px-3 py-3 text-xs font-bold text-blue-700 bg-blue-50 border-r border-blue-200">
                      IDR
                    </span>
                    <span className="px-4 py-3 text-base font-bold text-blue-900 text-right min-w-[160px] font-mono">
                      {totals.ppn.toLocaleString("en-US", {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
