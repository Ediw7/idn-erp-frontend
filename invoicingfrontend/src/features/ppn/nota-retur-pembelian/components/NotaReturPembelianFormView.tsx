...
import { SupplierData, MataUangData, ItemData, GudangData } from '../../../setup/api';
...
interface FormViewProps {
  ...
  updateLine: (idx: number, field: keyof NotaReturPembelianLine, value: any) => void;
}

export const NotaReturPembelianFormView: React.FC<FormViewProps> = ({
  form, setForm, isNew, suppliers, mataUangs, items, gudangs,
  dpp, ppnAmount,
  onSave, onDelete, onClose, onNew,
  handleSupplierChange, addLine, removeLine, updateLine
}) => {
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        ...
        <div className="bg-white border-l-4 border-l-blue-600 border-y border-r border-slate-300 rounded-sm shadow-sm p-4 shrink-0 flex justify-between items-center">
          <div className="flex gap-12">
            ...
            <div>
              <span className="block text-[10px] font-bold text-slate-500 uppercase">Penjual</span>
              <span className="text-base font-bold text-slate-800">{suppliers.find(p => p.id === form.supplier_id)?.nama || '-'}</span>
            </div>
            <div>
                ...
                <div className="flex gap-12">
                  ...
                  <div className="flex-1 flex flex-col gap-3 max-w-lg">
                    ...
                    <div className="flex items-start mt-2">
                      ...
                      <select className={inputClass} value={form.supplier_id || ''} onChange={e => handleSupplierChange(Number(e.target.value))}>
                        ...
                        {suppliers.map(p => <option key={p.id} value={p.id}>{p.nama}</option>)}
                      </select>
                    </div>
                    <div className="flex items-start">
                  ...
                  <div className="flex-1 flex flex-col gap-3 max-w-lg">
                    ...
                    <div className="flex items-start mt-2">
                      ...
                      <input type="text" className={inputClass} value={form.jenis_transaksi || ''} onChange={e => setForm({...form, jenis_transaksi: e.target.value})} />
                    </div>
                    <div className="flex items-start mt-2">
                  ...
                  <table className="w-full text-xs whitespace-nowrap">
                    <thead className="text-xs text-slate-700 bg-slate-100 border-b border-slate-200">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left border-r border-slate-200 min-w-[300px] font-semibold">Nama Barang Kena Pajak / Barang Mewah Yang Dikembalikan</th>
