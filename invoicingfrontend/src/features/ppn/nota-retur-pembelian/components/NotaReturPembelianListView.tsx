...
import { NotaReturPembelianData } from '../api';

interface ListViewProps {
  dataList: NotaReturPembelianData[];
  loadingData: boolean;
  ...
  onOpenForm: () => void;
  onEdit: (item: NotaReturPembelianData) => void;
  onDelete: (id: number) => void;
}

export const NotaReturPembelianListView: React.FC<ListViewProps> = ({
  dataList, loadingData,
  periode, setPeriode,
  onOpenForm, onEdit, onDelete
}) => {
  const [searchNoNota, setSearchNoNota] = React.useState('');
  const [searchPenjual, setSearchPenjual] = React.useState('');

  const filteredData = dataList.filter(item => {
    if (searchNoNota && !item.no_nota?.toLowerCase().includes(searchNoNota.toLowerCase())) return false;
    if (searchPenjual && !item.supplier_nama?.toLowerCase().includes(searchPenjual.toLowerCase())) return false;
    return true;
  });
      ...
      <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex justify-between items-center shrink-0">
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-4 mt-1.5">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-300 font-medium">No Retur:</span>
              <input type="text" className="text-xs bg-slate-700 text-white border border-slate-600 rounded-sm px-2 py-0.5 outline-none focus:border-slate-400 w-28" value={searchNoNota} onChange={e => setSearchNoNota(e.target.value)} />
            </div>
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-300 font-medium">Penjual:</span>
              <input type="text" className="text-xs bg-slate-700 text-white border border-slate-600 rounded-sm px-2 py-0.5 outline-none focus:border-slate-400 w-28" value={searchPenjual} onChange={e => setSearchPenjual(e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm overflow-x-auto flex-1">
          <table className="w-full text-xs text-left whitespace-nowrap">
            <thead className="bg-slate-100 border-b border-slate-200 sticky top-0 z-10">
              <tr>
                <th className="px-3 py-2 font-semibold text-slate-700">No. Nota Retur</th>
                ...
                <th className="px-3 py-2 font-semibold text-slate-700">Nama Penjual</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loadingData ? (
                <tr><td colSpan={8} className="text-center p-6 italic text-slate-500">Memuat data...</td></tr>
              ) : filteredData.length === 0 ? (
                <tr><td colSpan={8} className="text-center p-6 italic text-slate-500">Tidak ada data Nota Retur.</td></tr>
              ) : (
                filteredData.map((item, idx) => {
                  const dpp = (item.lines || []).reduce((acc, l) => acc + (l.harga_jual || 0), 0);
                  const ppn = dpp * (item.tarif_ppn || 11) / 100;
                  return (
                    <tr key={item.id || idx} className="hover:bg-slate-50 transition-colors">
                      <td className="px-3 py-2 font-mono text-blue-600 font-medium">{item.no_nota}</td>
                      <td className="px-3 py-2 text-slate-600">{item.tgl_nota}</td>
                      <td className="px-3 py-2 text-slate-800 font-mono text-[10px]">{item.atas_no_fp || ''}</td>
                      <td className="px-3 py-2 text-slate-800 truncate max-w-[200px]">{item.supplier_nama || ''}</td>
                      <td className="px-3 py-2 text-center text-slate-600 font-medium">{item.mata_uang_kode || 'IDR'}</td>
                      <td className="px-3 py-2 text-right font-mono text-slate-800">{dpp.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
                      <td className="px-3 py-2 text-right font-mono text-slate-800">{ppn.toLocaleString('en-US', { minimumFractionDigits: 2 })}</td>
