...
import { notaReturPembelianApi, NotaReturPembelianData, NotaReturPembelianLine } from '../api';
...
export const useNotaReturPembelianLogic = () => {
  ...
  const emptyForm: NotaReturPembelianData = {
    ...
    alamat_penjual: '', jenis_transaksi: 'Kepada Bukan Pemungut PPN (01)',
    gudang_id: '', jenis_retur: 'Barang Kena Pajak',
    atas_no_fp: '', tgl_fp: '', mata_uang_id: null,
    tarif_ppn: 11, kurs_pajak: 1, status: '',
    lines: [],
  ...
  const handleSupplierChange = (id: number | '') => {
    ...
    setForm({ ...form, supplier_id: id as number, alamat_penjual: s?.alamat || '' });
  };
  ...
  return {
    ...
    suppliers, mataUangs, items, gudangs,
    periode, setPeriode,
    form, setForm,
