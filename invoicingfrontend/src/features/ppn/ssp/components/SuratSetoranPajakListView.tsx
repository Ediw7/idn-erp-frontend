...
export const SuratSetoranPajakListView: React.FC<ListViewProps> = ({
  ...
  const filteredData = dataList.filter((item) => {
    ...
    if (
      searchJenisSetoran &&
      !item.kode_jenis_setoran.includes(searchJenisSetoran)
    )
      return false;
    return true;
  });

  const totalSsp = filteredData.reduce(
    ...
    <div className="bg-slate-50 shadow-sm border border-slate-300 flex flex-col h-[calc(100vh-8rem)]">
        ...
        <div className="bg-white border border-slate-200 rounded-sm shadow-sm overflow-x-auto flex-1 flex flex-col">
          <table className="w-full text-xs text-left whitespace-nowrap">
            <thead className="bg-slate-100 border-b border-slate-200 sticky top-0 z-10">
              <tr>
                <th className="px-3 py-2 font-semibold text-slate-700 text-center border-r border-slate-200">
                  Tanggal
                </th>
                <th className="px-3 py-2 font-semibold text-slate-700 text-left border-r border-slate-200">
                  Nama WP
                </th>
                <th className="px-3 py-2 font-semibold text-slate-700 text-center border-r border-slate-200">
                  Thn
                ...
                <th className="px-3 py-2 font-semibold text-slate-700 text-right border-r border-slate-200">
                  Jumlah
                </th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loadingData ? (
                ...
                filteredData.map((item, idx) => (
                  ...
                  >
                    <td className="px-3 py-2 text-center text-slate-600 border-r border-slate-100">
                      {item.tanggal}
                    </td>
                    <td className="px-3 py-2 text-left font-semibold text-slate-800 border-r border-slate-100">
                      {item.nama_wp}
                    </td>
                    <td className="px-3 py-2 text-center font-mono text-slate-700 border-r border-slate-100">
                      {item.tahun}
                    </td>
                    ...
                    <td className="px-3 py-2 text-center font-mono text-slate-700 border-r border-slate-100">
                      {item.kode_jenis_pajak}
                    </td>
                    <td className="px-3 py-2 text-center font-mono text-slate-700 border-r border-slate-100">
                      {item.kode_jenis_setoran}
                    </td>
                    <td
                      className="px-3 py-2 text-left text-slate-600 border-r border-slate-100 truncate max-w-xs"
                      title={item.uraian_pembayaran}
                    >
                      {item.uraian_pembayaran || "-"}
                    </td>
      ...
      <div className="bg-slate-50 border-t border-slate-200 px-6 py-3 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm font-bold text-slate-800 bg-slate-200 px-3 py-1 rounded-sm border border-slate-300">
            Total SSP:
          </span>
          <span className="text-sm font-bold text-white bg-slate-800 px-6 py-1 rounded-sm font-mono border border-slate-700">
            {totalSsp.toLocaleString("en-US")}
          </span>
        </div>
      </div>
    </div>
