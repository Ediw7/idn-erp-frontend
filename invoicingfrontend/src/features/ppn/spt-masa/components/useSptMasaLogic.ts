...
export const useSptMasaLogic = () => {
  ...
  const emptyForm: SptMasa1111Data = {
    ...
    dpp_ekspor: 0,
    ppn_ekspor: 0,
    dpp_dipungut_sendiri: 0,
    ppn_dipungut_sendiri: 0,
    dpp_dipungut_pemungut: 0,
    ppn_dipungut_pemungut: 0,
    dpp_tidak_dipungut: 0,
    ppn_tidak_dipungut: 0,
    dpp_dibebaskan: 0,
    ppn_dibebaskan: 0,
  ...
  const totalDppA = useMemo(() => {
    return (
      (form.dpp_ekspor || 0) +
      (form.dpp_dipungut_sendiri || 0) +
      (form.dpp_dipungut_pemungut || 0) +
      (form.dpp_tidak_dipungut || 0) +
      (form.dpp_dibebaskan || 0)
    );
  }, [
    form.dpp_ekspor,
    form.dpp_dipungut_sendiri,
    form.dpp_dipungut_pemungut,
    form.dpp_tidak_dipungut,
    form.dpp_dibebaskan,
  ]);
  ...
  const totalPpnA = useMemo(() => {
    return (
      (form.ppn_ekspor || 0) +
      (form.ppn_dipungut_sendiri || 0) +
      (form.ppn_dipungut_pemungut || 0) +
      (form.ppn_tidak_dipungut || 0) +
      (form.ppn_dibebaskan || 0)
    );
  }, [
    form.ppn_ekspor,
    form.ppn_dipungut_sendiri,
    form.ppn_dipungut_pemungut,
    form.ppn_tidak_dipungut,
    form.ppn_dibebaskan,
  ]);

  // Kalkulasi I.C
  const totalDppC = useMemo(
    () => totalDppA + (form.dpp_tidak_terutang || 0),
    [totalDppA, form.dpp_tidak_terutang],
  );
