...
export const useNotaReturPenjualanLogic = () => {
  ...
  const handlePelangganChange = (id: number | '') => {
    if (!id) {
      setForm({ ...form, pelanggan_id: null, alamat_pembeli: '', atas_no_invoice: '' });
      return;
    }
    const p = pelanggans.find(x => x.id === id);
    setForm({ ...form, pelanggan_id: id as number, alamat_pembeli: p?.alamat_kirim || p?.alamat || '', atas_no_invoice: '' });
  };
  ...
  return {
    ...
    pelanggans, mataUangs, items, gudangs, invoices,
    periode, setPeriode,
    form, setForm,
