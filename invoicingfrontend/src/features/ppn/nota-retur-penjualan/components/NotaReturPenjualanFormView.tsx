...
import { PelangganData, MataUangData, ItemData, GudangData } from '../../../setup/api';
...
export const NotaReturPenjualanFormView: React.FC<FormViewProps> = ({
  form, setForm, isNew, pelanggans, mataUangs, items, gudangs, invoices,
  dpp, ppnAmount,
  onSave, onDelete, onClose, onNew,
  handlePelangganChange, addLine, removeLine, updateLine
}) => {
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
        ...
        <div className="bg-white border-l-4 border-l-blue-600 border-y border-r border-slate-300 rounded-sm shadow-sm p-4 shrink-0 flex justify-between items-center">
          <div className="flex gap-12">
            <div>
              <span className="block text-[10px] font-bold text-slate-500 uppercase">No. Nota Retur</span>
              <span className="font-mono text-base font-bold text-slate-800">{form.no_nota || 'DRAFT'}</span>
            </div>
            <div>
            ...
            <div>
              ...
              <span className="text-base font-bold text-slate-800">{form.tgl_nota || '-'}</span>
            </div>
          </div>
        </div>
                ...
                <div className="flex gap-12">
                  ...
                  <div className="flex-1 flex flex-col gap-3 max-w-lg">
                    ...
                    <div className="flex items-start">
                      ...
                      <textarea className={`${readOnlyClass} h-16 resize-none`} readOnly value={form.alamat_pembeli || ''} />
                    </div>
                    <div className="flex items-start mt-2">
                  ...
                  <div className="flex-1 flex flex-col gap-3 max-w-lg">
                    ...
                    <div className="flex items-start mt-2">
                      ...
                      <select className={inputClass} value={form.atas_no_invoice || ''} onChange={e => setForm({...form, atas_no_invoice: e.target.value})}>
                        <option value="">- Pilih Invoice -</option>
                        {invoices.filter((i: any) => !form.pelanggan_id || i.pelanggan_id === form.pelanggan_id).map((inv: any) => (
                          <option key={inv.no_invoice || inv.id} value={inv.no_invoice || inv.id}>{inv.no_invoice || inv.id}</option>
                        ))}
                      </select>
                    </div>
                    <div className="flex items-start">
                ...
                <div className="overflow-x-auto min-h-[300px]">
                  <table className="w-full text-xs whitespace-nowrap">
                    <thead className="text-xs text-slate-700 bg-slate-100 border-b border-slate-200">
                      <tr>
                        <th className="w-10 px-2 py-2 text-center border-r border-slate-200 font-semibold">No</th>
                        <th className="w-48 px-3 py-2 text-left border-r border-slate-200 font-semibold">Kode Barang</th>
                        <th className="px-3 py-2 text-left border-r border-slate-200 min-w-[250px] font-semibold">Nama Barang Kena Pajak Yang Dikembalikan</th>
                        <th className="w-20 px-3 py-2 text-center border-r border-slate-200 font-semibold">Satuan</th>
                        <th className="w-24 px-3 py-2 text-right border-r border-slate-200 font-semibold">Kuantum</th>
                        <th className="w-32 px-3 py-2 text-right border-r border-slate-200 font-semibold">Harga Satuan</th>
                        <th className="w-32 px-3 py-2 text-right border-r border-slate-200 font-semibold">Harga Jual</th>
                        <th className="w-28 px-3 py-2 text-right border-r border-slate-200 font-semibold">HPP</th>
                        <th className="w-32 px-3 py-2 text-right border-r border-slate-200 font-semibold">Total HPP</th>
                        <th className="w-12 px-3 py-2 text-center font-semibold">Aksi</th>
                      </tr>
                    </thead>
                    <tbody>
                      {(form.lines || []).map((line, idx) => (
                        <tr key={idx} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                          <td className="px-2 py-1.5 text-center border-r border-slate-200 font-medium text-slate-500">{idx + 1}</td>
                          ...
                          <td className="px-2 py-1.5 border-r border-slate-200 text-right">
                            <input type="number" className="w-full px-2 py-1 border border-slate-300 rounded-sm text-xs text-right bg-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" value={line.kuantum || ''} onChange={e => updateLine(idx, 'kuantum', Number(e.target.value))} />
                          </td>
                          <td className="px-2 py-1.5 border-r border-slate-200 text-right">
                            <input type="number" className="w-full px-2 py-1 border border-slate-300 rounded-sm text-xs text-right bg-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" value={line.harga_satuan || ''} onChange={e => updateLine(idx, 'harga_satuan', Number(e.target.value))} />
                          </td>
                          <td className="px-2 py-1.5 border-r border-slate-200 text-right bg-slate-50">
                             <span className="font-semibold text-slate-800">{(line.harga_jual || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                          </td>
                          <td className="px-2 py-1.5 border-r border-slate-200 text-right">
                            <input type="number" className="w-full px-2 py-1 border border-slate-300 rounded-sm text-xs text-right bg-white focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500" value={line.hpp || ''} onChange={e => updateLine(idx, 'hpp', Number(e.target.value))} />
                          </td>
                          <td className="px-2 py-1.5 border-r border-slate-200 text-right bg-slate-50">
                            <span className="text-slate-600">{(line.total_hpp || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                          </td>
                          <td className="px-2 py-1.5 text-center">
                            <button onClick={() => removeLine(idx)} className="text-red-500 hover:text-red-700 p-1.5 rounded-sm transition-colors hover:bg-red-50" title="Hapus">
                              <Trash2 size={14}/>
                            </button>
                          </td>
                        </tr>
                      ...
                      <tr>
                        <td className="px-2 py-2 text-center border-r border-slate-200 text-slate-400 font-bold">*</td>
                        <td colSpan={9} className="px-3 py-2">
                          <button onClick={addLine} className="text-xs text-blue-600 font-semibold hover:text-blue-800 flex items-center gap-1 transition-colors">
          ...
          <div className="bg-slate-50 border-t border-slate-200 p-6 flex flex-col lg:flex-row gap-8 justify-between shrink-0">
            ...
            <div className="w-full lg:w-[500px] flex flex-col">
              <div className="flex flex-col gap-2 mb-6">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <div className="flex items-center gap-1 w-64">
                     <span className="w-16 text-center text-slate-500 font-medium">IDR</span>
                     <span className="flex-1 text-right font-mono font-bold text-slate-800 bg-slate-100 px-3 py-1.5 border border-slate-300 rounded-sm">{dpp.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
                <div className="flex justify-between items-center text-sm">
                  ...
                  <div className="flex items-center gap-1 w-64">
                     <span className="w-16 text-center text-slate-500 font-medium">IDR</span>
                     <input type="text" className={`${inputClass} flex-1 text-right font-mono font-semibold`} value="0.00" readOnly />
                  </div>
                </div>
                <div className="flex justify-between items-center text-sm">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <div className="flex items-center gap-1 w-64">
                     <span className="w-16 text-center text-slate-500 font-medium">IDR</span>
                     <span className="flex-1 text-right font-mono font-bold text-blue-700 bg-blue-50 px-3 py-1.5 border border-blue-200 rounded-sm">{dpp.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center text-sm pl-4">
                  ...
                  <div className="flex items-center gap-1 w-64">
                     <span className="w-16 text-center text-slate-500 font-medium">IDR</span>
                     <span className="flex-1 text-right font-mono font-bold text-blue-700 bg-blue-50 px-3 py-1.5 border border-blue-200 rounded-sm">{ppnAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                  </div>
                </div>
                <div className="flex justify-between items-center text-sm pl-4">
              ...
              <div className="flex justify-end gap-3 mt-2">
                <button disabled={isNew} onClick={onDelete} className="px-6 py-3 text-sm font-bold text-red-600 bg-white border border-red-200 hover:bg-red-50 transition-colors flex items-center justify-center gap-2 rounded-sm shadow-sm disabled:opacity-50">
                  <Trash2 size={16} /> HAPUS RETUR
                </button>
                <button onClick={onSave} className="px-8 py-3 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 transition-colors flex items-center justify-center gap-2 rounded-sm shadow-md w-full">
