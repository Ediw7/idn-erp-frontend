...
export const NotaReturPenjualanListView: React.FC<ListViewProps> = ({
  ...
  const filteredData = dataList.filter(item => {
    ...
    if (searchPembeli && !item.pelanggan_nama?.toLowerCase().includes(searchPembeli.toLowerCase())) return false;
    return true;
  });
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-4 mt-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <input type="text" className="text-xs bg-slate-700 text-white border border-slate-600 rounded-sm px-2 py-0.5 outline-none focus:border-slate-400 w-28" value={searchPembeli} onChange={e => setSearchPembeli(e.target.value)} />
            </div>
          </div>
        </div>
                ...
                filteredData.map((item, idx) => {
                  ...
                  return (
                    <tr key={item.id || idx} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-3 py-2 text-slate-800 truncate max-w-[200px]">{item.pelanggan_nama || ''}</td>
