...
const SetupUser: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || err.response?.data?.message || err.message || 'Gagal menyimpan data');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.error || err.response?.data?.message || err.message || 'Gagal menghapus pengguna');
    }
  };
    ...
    <div className="flex flex-col h-full w-full bg-white font-sans text-slate-800 relative">
      ...
      <div className="bg-slate-900 w-full rounded-none p-6 flex flex-row justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1">Kelola data pengguna dan hak akses sistem</p>
        </div>
        <button 
              ...
              {loading ? (
                ...
                users.map((user, idx) => (
                  <tr key={user.id} className="border-b border-gray-100 hover:bg-blue-50/50 transition-colors">
                    ...
                    <td className="px-5 py-3 font-semibold text-gray-800">{user.name}</td>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-gray-700">
                    Password {editId && <span className="text-gray-400 font-normal">(Kosongkan jika tidak ingin diubah)</span>}
                ...
                <div className="space-y-1">
                  <label className="text-xs font-semibold text-gray-700">Role / Hak Akses <span className="text-red-500">*</span></label>
                  <select 
                    value={formData.role}
