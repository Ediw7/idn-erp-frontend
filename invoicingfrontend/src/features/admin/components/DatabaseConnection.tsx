...
const DatabaseConnection: React.FC = () => {
  ...
  const handleSave = async () => {
    const isConfirmed = await confirm(
      "Peringatan Keamanan: Menyimpan konfigurasi baru akan merestart koneksi server backend Anda secara otomatis. Lanjutkan?",
    );
    ...
    try {
      ...
      toast.success(
        "Konfigurasi berhasil disimpan! Server sedang direstart...",
      );
      setIsEditing(false);
      ...
      <div className="bg-slate-900 w-full rounded-none p-6 flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1">
            Konfigurasi aman koneksi ke PostgreSQL server
          </p>
        </div>
      </div>
              ...
              <div className="space-y-1">
                ...
                {!isEditing && (
                  <p className="text-[11px] text-gray-400 mt-1 italic">
                    Password disensor untuk alasan keamanan arsitektur.
                  </p>
                )}
              </div>
            </div>
