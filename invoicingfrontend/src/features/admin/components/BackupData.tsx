...
const BackupData: React.FC = () => {
  ...
  const handleBackup = async () => {
    ...
    const toastId = toast.loading(
      "Sedang men-generate file backup database...",
    );
    ...
    try {
      ...
      toast.success("Backup berhasil diunduh ke komputer Anda!", {
        id: toastId,
      });
      ...
      <div className="bg-slate-900 w-full rounded-none p-6 flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1">
            Unduh salinan database sistem untuk keamanan data
          </p>
        </div>
      </div>
        ...
        <div className="max-w-4xl bg-white border border-gray-200 shadow-sm rounded-sm p-8">
          <div className="bg-blue-50 border border-blue-200 p-6 rounded-sm flex items-start gap-4 mb-8">
            ...
            <div>
              <h3 className="text-sm font-bold text-blue-900 mb-1">
                Mekanisme Backup Web App
              </h3>
              <p className="text-sm text-blue-800 leading-relaxed">
                Proses backup akan menginstruksikan server backend (PostgreSQL)
                untuk men-generate file ekspor (SQL/ZIP) dari seluruh tabel dan
                skema database saat ini. Setelah siap, file tersebut akan
                disalurkan via internet dan{" "}
                <strong>otomatis terunduh langsung ke komputer Anda</strong>{" "}
                layaknya mengunduh file biasa. Simpan file tersebut di lokasi
                yang aman.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-gray-300 rounded-sm bg-gray-50">
            ...
            <h2 className="text-lg font-bold text-gray-700 mb-2">
              Siap untuk Backup Database?
            </h2>
            <p className="text-sm text-gray-500 mb-6 text-center max-w-md">
              Pastikan tidak ada transaksi besar yang sedang berjalan di sistem
              sebelum melakukan backup manual untuk meminimalisir data yang
              terlewat.
            </p>
