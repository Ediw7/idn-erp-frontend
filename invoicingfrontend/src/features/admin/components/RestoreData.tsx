  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (!file.name.endsWith('.sql') && !file.name.endsWith('.zip')) {
        toast.error('Format file tidak didukung! Harap unggah file .sql atau .zip.');
        return;
      }
    ...
    <div className="flex flex-col w-full h-full bg-white font-sans text-slate-800">
      ...
      <div className="bg-slate-900 w-full rounded-none p-6 flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1">Pulihkan database sistem dari file salinan backup</p>
        </div>
      </div>
      ...
      <div className="p-8 flex-1 overflow-y-auto bg-gray-50 flex flex-col items-center">
        ...
        <div className="w-full max-w-3xl">
          ...
          <div className="bg-red-50 border-l-4 border-red-600 p-5 rounded-r-md shadow-sm mb-8 flex items-start gap-4">
            ...
            <div>
              <h3 className="text-base font-bold text-red-800 mb-1">PERINGATAN KRITIKAL</h3>
              <p className="text-sm text-red-700 leading-relaxed font-medium">
                Melakukan Restore akan <strong>MENGHAPUS dan MENIMPA</strong> seluruh data sistem saat ini dengan data dari file backup. Pastikan file yang Anda unggah benar dan valid. Tindakan ini <u>tidak dapat dibatalkan</u> dan pengguna lain akan ter-logout otomatis!
              </p>
            </div>
          </div>

          <div className="bg-white border border-gray-200 shadow-sm rounded-lg p-10">
            <h2 className="text-lg font-bold text-gray-800 mb-6 text-center">Unggah File Backup</h2>
            ...
            {!selectedFile ? (
              ...
              >
                ...
                <h3 className="text-base font-bold text-gray-700 mb-2">Klik atau Drag & Drop file backup di sini</h3>
                <p className="text-sm text-gray-500">Mendukung format file terkompresi .sql atau .zip</p>
              </div>
            ) : (
              ...
              <div className="border border-green-200 bg-green-50 p-6 rounded-lg text-center relative transition-all">
                ...
                <h3 className="text-base font-bold text-green-800 mb-1 line-clamp-1" title={selectedFile.name}>
                  {selectedFile.name}
                </h3>
                ...
                {isRestoring ? (
                  <div className="w-full max-w-sm mx-auto">
                    ...
                    <p className="text-xs text-green-700 mt-2">Mohon tunggu, jangan tutup halaman ini.</p>
                  </div>
                ) : (
                  <button 
            ...
            <div className="mt-8 pt-6 border-t border-gray-100">
              <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                <CheckCircle2 size={16} className="text-green-500" /> Standar Keamanan Restore Web:
              </h4>
              <ul className="text-xs text-gray-500 space-y-2 list-disc list-inside ml-1">
                <li>Sistem hanya menerima file enkripsi terkompresi dari panel Backup resmi aplikasi ini.</li>
                <li>Semua sesi pengguna lain akan dihentikan paksa (force logout) saat proses berjalan.</li>
                <li>Koneksi backend akan di-*restart* secara instan setelah file sukses terurai di dalam server.</li>
              </ul>
            </div>
