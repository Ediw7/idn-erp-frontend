...
const SetupUserPermission: React.FC = () => {
  ...
  const [permissions, setPermissions] =
    useState<PermissionData[]>(initialPermissions);
  ...
  const handleGrantFullAccess = () => {
    ...
    toast.success(
      "Akses penuh diberikan ke seluruh sistem! (Tekan Simpan untuk memperbarui database)",
    );
  };
    ...
    <div className="flex flex-col w-full h-full bg-white font-sans text-slate-800">
      ...
      <div className="bg-slate-900 w-full rounded-none p-6 flex justify-between items-center shrink-0">
        <div>
          ...
          <p className="text-sm text-slate-300 mt-1">
            Atur hak akses tiap pengguna secara detail
          </p>
        </div>
      ...
      <div className="flex flex-row w-full flex-1 overflow-hidden">
        ...
        <div className="w-3/4 p-6 overflow-y-auto bg-white">
          {isAdminSelected && (
            <div className="bg-blue-50 border border-blue-200 text-blue-800 px-5 py-4 rounded-sm mb-6 flex items-start gap-3 shadow-sm">
              ...
              <div className="text-sm">
                ...
                <p>
                  Pengguna ini adalah Administrator dan otomatis memiliki akses
                  penuh ke seluruh fitur dan pengaturan sistem. Konfigurasi hak
                  akses granular (seperti form, laporan, dan proses) tidak
                  diperlukan dan telah dinonaktifkan.
                </p>
              </div>
            </div>
          )}
          ...
          <div
            className={`border border-gray-200 rounded-sm overflow-hidden shadow-sm ${isAdminSelected ? "opacity-50 grayscale pointer-events-none" : ""}`}
          >
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 text-gray-700 font-semibold border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-5 py-3 border-r border-gray-200 text-center w-24">
                    Open
                  </th>
                  <th className="px-5 py-3 border-r border-gray-200 text-center w-24">
                    Insert
                    ...
                    >
                      ...
                      <td className="px-5 py-2.5 border-r border-gray-100 text-center">
                        <input
                          ...
                          onChange={(e) =>
                            handleCheckboxChange(p.id, "open", e.target.checked)
                          }
                          disabled={isAdminSelected}
