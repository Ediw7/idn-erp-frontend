...
const NotaKredit: React.FC = () => {
  ...
  const totalNilai = useMemo(() => {
    return details.reduce((sum, row) => sum + parseCurrency(row.jumlahStr || '0'), 0);
  }, [details]);
  ...
  const addRow = () => {
    setDetails([...details, { id: Date.now().toString(), keterangan: '', jumlahStr: '', noPerkiraan: '' }]);
  };
  ...
  const removeRow = (id: string) => {
    if (details.length > 1) {
      ...
      setDetails([{ id: Date.now().toString(), keterangan: '', jumlahStr: '', noPerkiraan: '' }]);
    }
  };
          ...
          <div className="flex gap-12">
            ...
            <div className="flex-1 flex flex-col gap-2 max-w-lg">
              ...
              <div className="flex items-center mt-2">
                ...
                <div className="flex gap-1 w-64 items-center">
                  ...
                  <input type="text" className={`${readOnlyClass} flex-1 text-right font-bold`} value={formatCurrency(totalNilai)} readOnly />
                </div>
              </div>
            ...
            <div className="flex-1 flex flex-col gap-2 max-w-lg">
              ...
              <div className="flex items-center">
                ...
                <input type="text" className={inputClass} value={selectedTtd?.jabatan || ''} onChange={() => {}} />
              </div>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-xs whitespace-nowrap">
              <thead className="text-xs text-slate-700 bg-slate-100 border-b border-slate-300">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left border-r border-slate-300 min-w-[300px] font-semibold">Keterangan</th>
                  <th className="w-48 px-3 py-2 text-right border-r border-slate-300 font-semibold">Jumlah</th>
                  <th className="w-48 px-3 py-2 text-left border-r border-slate-300 font-semibold">No Perkiraan</th>
              ...
              <tbody>
                {details.map((row, index) => (
                  <tr key={row.id} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                    <td className="px-2 py-1 text-center border-r border-slate-200">{index + 1}</td>
                    ...
                    <td className="px-2 py-1 border-r border-slate-200 text-right">
                      <input 
                        ...
                        onFocus={(e) => handleNumberFocus(row.id, e.target.value)}
                      />
                    </td>
                    <td className="px-2 py-1 border-r border-slate-200">
                      <select 
                        ...
                        onChange={(e) => handleRowChange(row.id, 'noPerkiraan', e.target.value)}
                      >
