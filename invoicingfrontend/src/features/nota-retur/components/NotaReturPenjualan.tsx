...
const NotaReturPenjualan: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      const [p, m, i, g, invRes] = await Promise.all([
        setupApi.getPelanggan(),
        setupApi.getMataUang(),
        setupApi.getItem(),
        setupApi.getGudang().catch(() => []),
        axiosClient.post('/api/saldo-awal-piutang/get', { jsonrpc: '2.0', params: {} }).catch(() => ({ data: { result: { data: [] } } }))
      ]);
      ...
      if (invRes.data.result && invRes.data.result.data) {
        setInvoices(invRes.data.result.data);
      }
  ...
  const handlePelangganChange = (id: number) => {
    ...
    setForm({ ...form, pelanggan_id: id, alamat_pembeli: p?.alamat_kirim || p?.alamat || '', atas_no_invoice: '' });
  };
  ...
  return (
    <div className="bg-white shadow-sm border border-slate-300 flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="flex-1 overflow-y-auto p-4 flex flex-col">
            ...
            <div className="flex-1 flex flex-col gap-2 max-w-lg">
              ...
              <div className="flex items-center mt-2">
                ...
                <select className={inputClass} value={form.atas_no_invoice || ''} onChange={e => setForm({...form, atas_no_invoice: e.target.value})}>
                  ...
                  {invoices.filter(i => !form.pelanggan_id || i.pelanggan_id === form.pelanggan_id).map(inv => (
            ...
            <table className="w-full text-xs whitespace-nowrap">
              <thead className="text-xs text-slate-700 bg-slate-100 border-b border-slate-300">
                <tr>
                  ...
                  <th className="px-3 py-2 text-left border-r border-slate-300 min-w-[250px] font-semibold">Nama Barang Kena Pajak Yang Dikembalikan</th>
                  <th className="w-20 px-3 py-2 text-center border-r border-slate-300 font-semibold">Satuan</th>
                  <th className="w-24 px-3 py-2 text-right border-r border-slate-300 font-semibold">Kuantum</th>
                  <th className="w-32 px-3 py-2 text-right border-r border-slate-300 font-semibold">Harga Satuan</th>
                  <th className="w-32 px-3 py-2 text-right border-r border-slate-300 font-semibold">Harga Jual</th>
                  <th className="w-28 px-3 py-2 text-right border-r border-slate-300 font-semibold">HPP</th>
                  <th className="w-32 px-3 py-2 text-right border-r border-slate-300 font-semibold">Total HPP</th>
              ...
              <tbody>
                {(form.lines || []).map((line, idx) => (
                  <tr key={idx} className="border-b border-slate-200 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-2 py-1 border-r border-slate-200 text-right">
                      <input type="number" className="w-full px-2 py-1 border border-slate-300 rounded-sm text-xs text-right bg-white focus:outline-none focus:ring-1 focus:ring-slate-500" value={line.hpp || ''} onChange={e => updateLine(idx, 'hpp', Number(e.target.value))} />
                    </td>
                    <td className="px-2 py-1 border-r border-slate-200 text-right">
                      <input type="text" className={`${readOnlyClass} !px-2 !py-1 text-right text-slate-500`} readOnly value={(line.total_hpp || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })} />
                    </td>
        ...
        {/* Footer: Informasi Cetak & Kalkulasi Pajak */}
        <div className="bg-white rounded-md shadow-sm border border-slate-200 p-4 shrink-0 grid grid-cols-1 xl:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-2 justify-end">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <div className="flex items-center gap-1 w-64">
                 <input type="text" className={`${readOnlyClass} w-16 text-center font-bold`} value="IDR" readOnly />
                 <input type="text" className={`${inputClass} flex-1 text-right font-semibold`} value="0.00" readOnly />
              </div>
            </div>
            <div className="flex justify-between items-center text-sm">
